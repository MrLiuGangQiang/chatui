...
function readGlobal(path, root = globalThis) {
  const direct = String(path || '').split('.').filter(Boolean).reduce((target, key) => (target && target[key] ? target[key] : undefined), root);
  if (direct !== undefined) return direct;
  return GLOBAL_ALIASES[path] ? readGlobal(GLOBAL_ALIASES[path], root) : undefined;
}

function hasGlobal(path, root = globalThis) { return readGlobal(path, root) !== undefined; }

function getReadiness(root = globalThis) {
  const scripts = resources.scripts.reduce((result, resource) => {
    result[resource.id] = hasGlobal(resource.global, root);
    return result;
  }, {});
  return { version: VERSION, scripts, ready: Object.values(scripts).every(Boolean) };
}

function createBrowserLoader(root = globalThis, doc = root.document) {
  const loadState = new Map();
  const log = (level, message, detail) => {
    try { (root.console?.[level] || root.console?.log || (() => {})).call(root.console, message, detail || ''); } catch {}
  };
  const appendNode = (node) => (node.tagName === 'LINK' ? doc.head : doc.body || doc.head).appendChild(node);

  function markExisting(resource) {
    const element = doc?.querySelector?.(`[data-markdown-dependency="${resource.id}"]`);
    if (element && !element.dataset.markdownDependencyLoaded) element.dataset.markdownDependencyLoaded = 'global';
  }

  function loadElement(resource, createNode) {
    if (!doc) return Promise.resolve({ id: resource.id, from: 'no-document' });
    if (resource.global && hasGlobal(resource.global, root)) {
      markExisting(resource);
      return Promise.resolve({ id: resource.id, from: 'global' });
    }
    ...
    const promise = new Promise((resolve, reject) => {
      ...
      const attempt = (url, from) => {
        cleanup();
        node = createNode(url);
        node.dataset.markdownDependency = resource.id;
        ...
        node.onerror = () => {
          log('warn', `[markdown] dependency failed: ${resource.id} (${from})`, url);
          if (from === 'cdn' && resource.local) { attempt(resource.local, 'local'); return; }
          cleanup();
          reject(new Error(`Failed to load markdown dependency: ${resource.id}`));
        };
        timer = setTimeout(() => node.onerror(), DEFAULT_TIMEOUT_MS);
        appendNode(node);
      };
      attempt(resource.cdn, 'cdn');
    }).catch((err) => {
      loadState.delete(resource.id);
      throw err;
    });
    loadState.set(resource.id, promise);
    return promise;
  }
  ...
  const loadScripts = () => resources.scripts.reduce((chain, resource) => chain.then(list => loadScript(resource).then(item => list.concat(item)).catch(err => {
    log('warn', `[markdown] optional dependency unavailable: ${resource.id}`, err);
    return list.concat({ id: resource.id, error: err.message || String(err) });
  })), Promise.resolve([]));
...
module.exports = { VERSION, DEFAULT_TIMEOUT_MS, resources, readGlobal, hasGlobal, getReadiness, createBrowserLoader };
