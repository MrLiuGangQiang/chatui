...
function testIntentRecognitionUsesABoundedNoToolRequestWithThinkingDisabled() {
  ...
  assert.ok(system);
  assert.ok(system.content.length <= 7600, `route system prompt must remain bounded, got ${system.content.length} characters`);
...
function testIntentRecognitionPromptStatesReadableDecisionPriority() {
  ...
  for (const section of ['current_input', 'operation', 'task_shape', 'resource_refs', 'relation', 'goal', 'goal_mode', 'auto_mode/current_mode']) {
