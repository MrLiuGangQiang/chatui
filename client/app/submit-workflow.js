            ...
            if(storedPending&&clarification.buildContinuationClassifierPayload&&clarification.parseContinuationClassifierResult&&typeof getConfig==="function"&&typeof requestJson==="function"){
              const cfg=getConfig(),model=typeof getSessionRouteModel==="function"?getSessionRouteModel(sessionId,cfg):cfg.routeModel||cfg.chatModel;
               if(cfg.baseUrl&&model){const classifierDeadline=createBoundedIntentRequest(run.abortController?.signal,intentDeadlineAt);try{
                 ...
                 let result;try{result=await requestClassifier(payload)}catch(err){if(!payload.response_format||!structuredOutputUnsupported(err))throw err;const compatibilityPayload={...payload};delete compatibilityPayload.response_format;result=await requestClassifier(compatibilityPayload)}
                 pendingDecision=clarification.parseContinuationClassifierResult(result?.choices?.[0]?.message?.content||result?.output_text||"",{pending:storedPending});
               }catch(err){if(err?.code==="ROUTE_INTENT_TIMEOUT")throw err;console.warn("continuation classifier failed; treating current input as a new task:",err)}finally{classifierDeadline.dispose()}}
            }
