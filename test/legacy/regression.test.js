...
function testDownloadFilenamesUseShanghaiTimestamp() {
  ...
  assert.ok(index.includes('client/ui/file-actions.js?v=1.2.67') && index.includes('client/app/image-actions-workflow.js?v=1.2.68') && index.includes('client/app/image-result-workflow.js?v=1.7.1-image-semantics') && index.includes('app.js?v=2.1.38-multi-image-lifecycle') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'browser cache versions should match the current static bundle');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match browser cache-busting');
}
...
function testPendingClarificationClearsAfterMergedSend() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('submit-workflow.js?v=1.2.87-message-size-guard'), 'submit workflow cache version should be bumped for pending clarification fix');
...
function testChatAnswerStreamingFlushesQuickly() {
  ...
  assert.ok(index.includes('chat-workflow.js?v=1.3.24-task-lifecycle-state-machine') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && bundle.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'cache-busting versions should be bumped for streaming performance fixes');
}
...
function testStreamingTailRendersWithoutCursor() {
  ...
  assert.ok(index.includes('browser-streaming-renderer.js?v=1.2.91') && index.includes('message-workflow.js?v=1.3.39-image-quote') && index.includes('flat-theme.css?v=2.1.88-content-region-dialogs'), 'cache-busting versions should be bumped after removing the streaming cursor');
}
...
function testEmptySessionEmbedsRouteDiagram() {
  const app = fs.readFileSync(path.join(__dirname, '../../app.js'), 'utf8');
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  const css = fs.readFileSync(path.join(__dirname, '../../styles/flat-theme.css'), 'utf8');
  const routePage = fs.readFileSync(path.join(__dirname, '../../route.html'), 'utf8');
  ...
  assert.ok(!index.includes('empty-route-diagram'), 'static HTML should not render the embedded diagram before session restore');
  ...
  assert.ok(app.includes('class="empty-route-diagram"') && app.includes('class="empty-route-diagram-frame"'), 'empty sessions should render the intent-recognition diagram container and iframe');
  assert.ok(app.includes('src="./route.html?v=1.1.3-title-map"') && app.includes('loading="eager"'), 'the embedded diagram should load the cache-busted route page immediately');
  assert.ok(!app.includes('welcome-title') && !app.includes('welcome-feature-grid') && !app.includes('welcome-guidelines'), 'the previous welcome title and capabilities should remain removed');
  assert.ok(app.includes('class="empty-route-guidelines"') && app.includes('\u4f7f\u7528\u987b\u77e5') && app.includes('\u5185\u5bb9\u5b89\u5168') && app.includes('\u672c\u5730\u5b58\u50a8') && app.includes('\u5408\u89c4\u4f7f\u7528'), 'the previous usage notice content should be restored below the embedded diagram');
  ...
  assert.ok(index.includes('./app.js?v=2.1.38-multi-image-lifecycle') && index.includes('flat-theme.css?v=2.1.88-content-region-dialogs'), 'embedded diagram assets should use updated cache versions');
  ...
  assert.ok(css.includes('grid-template-columns:repeat(3,minmax(0,1fr))') && css.includes('grid-template-columns:1fr!important'), 'usage guidance should use three columns on desktop and stack on mobile');
  assert.ok(!css.includes('.welcome-title') && !css.includes('.welcome-feature-grid') && !css.includes('.welcome-guidelines'), 'obsolete welcome presentation styles should stay removed');
  assert.ok(!routePage.includes("classList.add('embedded')") && !routePage.includes('min-width: 1120px') && !routePage.includes('min-width: 980px'), 'the route page should fit the iframe without embedded scaling or scrollbars');
  assert.ok(routePage.includes('>ChatUI</text>') && routePage.includes('fill="url(#titleGrad)">\u00b7</text>') && routePage.includes('>\u5965\u54f2AI\u52a9\u624b\uff1a</text>') && routePage.includes('>12\u6b65\u6267\u884c\u5730\u56fe</text>'), 'the route title should preserve ChatUI, the colored middle dot, the Aozhe AI assistant name, and the 12-step execution map label');
}
...
function testQuotePreviewIsFeatureModule() {
  ...
  assert.ok(index.includes('message-workflow.js?v=1.3.39-image-quote') && index.includes('message-model.js?v=1.0.1') && index.includes('message-domain.js?v=1.0.1') && index.includes('styles/messages.css?v=1.3.17-web-preview-iframe-borderless') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'quote filtering and jump flash changes should bump cache versions');
...
function testHistoryAnchorLastQuestionSpacerClearsOnSubmit() {
  ...
  assert.ok(index.includes('history-anchor-nav.js?v=1.0.18') && index.includes('submit-workflow.js?v=1.2.87-message-size-guard') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'history spacer submit fix should bump browser cache versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match the directory spacer fix cache-busting');
}
...
function testRegenerateSavesEarlyPendingSubmitBeforeRoute() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('regenerate-workflow.js?v=1.0.2-canonical-route-plan') && index.includes('app.js?v=2.1.38-multi-image-lifecycle') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'cache versions should deliver the canonical regenerate workflow');
}
...
function testReasoningCompletesBeforeAnswerStreaming() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('chat-workflow.js?v=1.3.24-task-lifecycle-state-machine') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'chat stream reasoning-state fix should bump cache versions');
}
...
function testReasoningUnavailableWhenAnswerStartsWithoutReasoning() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('chat-workflow.js?v=1.3.24-task-lifecycle-state-machine') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'empty-reasoning stream fix should bump cache versions');
}
...
function testReasoningMenuCloseReleasesFocusBeforeAriaHidden() {
  ...
  assert.ok(index.includes('reasoning-workflow.js?v=1.3.35-reasoning-persistence') && index.includes('composer.css?v=1.3.2-gpt5-reasoning-menu') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'reasoning menu changes should bump cache versions');
}
...
function testCodeActionHoverAndHistoryAnchorActivePolish() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs') && index.includes('assets/chatui.bundle.css?v=1.3.125-image-composition-passthrough'), 'CSS bundle cache versions should be bumped for visual fixes');
}
...
function testConfigCopyButtonsForBaseUrlAndApiKey() {
  ...
  assert.ok(index.includes('config-workflow.js?v=1.2.73-configurable-upstream') && index.includes('bootstrap-workflow.js?v=2.1.0-preinsert-guard') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs') && index.includes('app.js?v=2.1.38-multi-image-lifecycle') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough'), 'config copy UI changes should bump browser cache versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match config copy cache-busting');
}
...
function testForceImageButtonOnUserMessages() {
  ...
  assert.ok(index.includes('message-workflow.js?v=1.3.39-image-quote') && index.includes('regenerate-workflow.js?v=1.0.2-canonical-route-plan') && index.includes('app.js?v=2.1.38-multi-image-lifecycle') && index.includes('assets/chatui.bundle.css?v=1.3.125-image-composition-passthrough') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs'), 'force-image UI and action changes should bump cache-busting versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match force-image cache-busting');
}
...
function testImagePreviewWheelZoom() {
  ...
  assert.ok(index.includes('image-preview-workflow.js?v=1.2.67') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs'), 'image preview zoom should bump cache-busting versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match image preview zoom bundle cache-busting');
}
...
function testMessageActionButtonsUsePolishedStyle() {
  ...
  assert.ok(index.includes('assets/chatui.bundle.css?v=1.3.125-image-composition-passthrough') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs'), 'message action visual polish should bump cache-busting versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match message action polish cache-busting');
}
...
function testPendingFeedbackDoesNotWrapOnMobile() {
  ...
  assert.ok(index.includes('assets/chatui.bundle.css?v=1.3.125-image-composition-passthrough') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs'), 'pending feedback mobile nowrap fix should bump cache-busting versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match pending feedback cache-busting');
}
...
function testComposerWidthFollowsSidebarCollapsedMessageColumn() {
  ...
  assert.ok(index.includes('assets/chatui.bundle.css?v=1.3.125-image-composition-passthrough') && index.includes('chatui.bundle.js?v=1.3.125-image-composition-passthrough') && index.includes('styles/flat-theme.css?v=2.1.88-content-region-dialogs'), 'sidebar composer width fix should bump browser cache versions');
  assert.ok(bundleSource.includes("BUNDLE_VERSION = '1.3.125-image-composition-passthrough'"), 'server bundle version should match sidebar composer cache-busting');
}
...
function testRouteTimeoutShowsSlowNoticeThenFailsCleanly() {
  ...
  assert.ok(index.includes('submit-workflow.js?v=1.2.87-message-size-guard') && index.includes('chat-workflow.js?v=1.3.24-task-lifecycle-state-machine') && index.includes('route-decision-workflow.js?v=2.0.0-task-contract-v3') && index.includes('app.js?v=2.1.38-multi-image-lifecycle') && index.includes('flat-theme.css?v=2.1.88-content-region-dialogs'), 'cache versions should be bumped for route timeout UX');
}
...
function testImageSuccessResultReconciliation() {
  ...
  assert.ok(index.includes('image-result-reconciliation.js?v=1.0.0') && index.indexOf('image-result-reconciliation.js?v=1.0.0') < index.indexOf('app.js?v=2.1.38-multi-image-lifecycle'), 'reconciliation module should load before app.js');
  assert.ok(index.includes('image-workflow.js?v=1.3.20-edit-resend-tdz') && index.includes('job-resume-workflow.js?v=1.2.76-background-settle') && index.includes('app.js?v=2.1.38-multi-image-lifecycle'), 'image success reconciliation should bump workflow and app cache versions');
}
...
const tests = [
  ...
  testEmptySessionEmbedsRouteDiagram,
