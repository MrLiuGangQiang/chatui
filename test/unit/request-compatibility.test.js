...
async function testJsonObjectFallbackIncludesRequiredLowercaseJsonKeyword() {
  ...
  const result = await compatibility.requestJsonWithStructuredOutputFallback(async payload => {
    ...
    if (attempts.length === 1) {
      const error = new Error("Invalid schema for response_format 'chatui_route_intent_v2': additionalProperties is required to be supplied and to be false.");
    ...
    model: 'route-model',
    response_format: { type: 'json_schema', json_schema: { name: 'chatui_route_intent_v2', schema: { type: 'object' } } },
...
async function testStructuredOutputFallbackRetriesInvalidProviderSchema() {
  ...
  const result = await compatibility.requestJsonWithStructuredOutputFallback(async payload => {
    attempts.push(payload);
    if (attempts.length === 1) {
      const error = new Error("Invalid schema for response_format 'chatui_route_intent_v2': 'uniqueItems' is not permitted.");
      error.code = 'invalid_json_schema';
      throw error;
    }
    return { choices: [{ message: { content: '{}' } }] };
...
function testStructuredOutputFallbackClassifierRecognizesOnlyProtocolCapabilityErrors() {
  ...
  assert.strictEqual(compatibility.structuredOutputUnsupported({
    ...
    message: "Invalid schema for response_format 'chatui_route_intent_v2': 'uniqueItems' is not permitted.",
  }), true);
