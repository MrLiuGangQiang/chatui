...
- 输入内容后自动判断是聊天任务还是生图 / 改图任务。
- 优先使用本地规则识别明确意图，减少额外请求。
- 本地规则不确定时，会调用已配置的聊天模型做轻量路由分类，只要求模型返回 `chat` 或 `image`。
- 如果没有配置聊天模型，或路由模型请求失败，则兜底按聊天任务处理。
- 支持中文和英文常见生图表达识别。
- 上传图片并输入“改图、换背景、修图、让这张图……”等描述时，会优先进入图片编辑流程。
- 上传非图片附件时，优先进入聊天模式并把可解析文本作为上下文。
