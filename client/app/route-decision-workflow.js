(function initChatUIAppRouteDecisionWorkflow(root) {
  ...
  function createRouteDecisionWorkflow(deps = {}) {
    ...
    async function getEffectiveRoute(e,t=state.attachments,s=state.activeSessionId,h=null,routeContextOverride=null,routeOptions=null) {
      with (deps) {
        const n=getConfig(),r=h||buildRequestHeaders("message",s),a=n.routeModel||n.chatModel,routeSvc=window.ChatUIServices?.route||window.ChatUIRouteService,attachmentMeta=buildRouteAttachmentMetadata(t),context=routeContextOverride||buildRouteContext(8,s);if(n.baseUrl&&a)try{const h=routeSvc?.buildRoutePayload?routeSvc.buildRoutePayload({model:a,input:e,attachments:attachmentMeta,context,currentMode:state.mode,autoMode:state.autoMode}):{model:a,temperature:0,messages:[]},controller=typeof AbortController!=="undefined"?new AbortController:null;let timedOut=!1,slowNotified=!1;const slowTimer=setTimeout(()=>{slowNotified=!0;try{routeOptions?.onSlow?.()}catch(e){console.warn("route slow callback failed:",e)}},10000),timeout=setTimeout(()=>{timedOut=!0;controller?.abort?.()},60000);let i;try{i=await requestJson(`${n.baseUrl}/chat/completions`,h,n.apiKey,{headers:r,signal:controller?.signal})}catch(err){if(timedOut||"AbortError"===err?.name){const timeoutError=new Error("ROUTE_INTENT_TIMEOUT");timeoutError.code="ROUTE_INTENT_TIMEOUT";timeoutError.routeTimedOut=!0;timeoutError.timeoutMs=60000;timeoutError.slowNotified=slowNotified;throw timeoutError}throw err}finally{clearTimeout(slowTimer),clearTimeout(timeout)}const o=parseRouteResult(routeSvc?.extractRouteText?routeSvc.extractRouteText(i):i?.choices?.[0]?.message?.content||i?.output_text||"",{input:e,attachments:attachmentMeta,context});if(o){const prompt=String(context?.suggested_contextual_image_prompt||"").trim();return"image"===o.mode&&prompt&&!o.contextualImagePrompt?{...o,contextualImagePrompt:prompt}:o}}catch(e){if(e?.routeTimedOut||"ROUTE_INTENT_TIMEOUT"===e?.code)throw e;console.warn("model route failed, fallback to chat:",e)}return normalizeRoute({mode:"chat",target:"none",use_previous_image:!1,confidence:0,evidence:"意图识别模型不可用，默认走聊天模型"},"chat")
      }
    }
