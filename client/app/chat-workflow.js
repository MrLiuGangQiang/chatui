(function initChatUIAppChatWorkflow(root) {
  ...
  const DEFAULT_CHAT_OUTPUT_POLICY = [
    ...
    '当用户要求筛选、删除、压缩或只保留某个优先级/部分时，把上一条答案当作唯一候选集合，只输出筛选后保留的内容，不要重新扩展清单。若用户要求只保留发布前的 P0 项，最终答案只能包含不满足就必须阻止上线的发布前条件；绝对删除并禁止输出负责人、责任人、审批人、值守、值班、观察窗口、发布后观察、上线后验证、监控、告警、日志、通知、发布流程、发布窗口、运营或其他运维安排。若一条同时包含允许内容和上述禁止内容，整条删除；不要重新生成最低门槛、附录或解释。',
  ].join('\n');
