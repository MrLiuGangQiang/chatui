...
## 图片预览压缩下载
`client/app/image-preview-workflow.js` 使用浏览器 Canvas，保持原尺寸和格式，不修改持久化原图或新增服务端接口。输入为目标体积比例，PNG 仅无损重编码，质量可调格式采用有界搜索。不支持的编码不得静默转换。详见 `docs/design/14-图片预览压缩下载设计.md`。
