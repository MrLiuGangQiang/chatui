...
- 意图/图片提示词集中在 `route-prompts.js`；强事实归一化在 `route-semantic-normalizer.js`；历史图片检索在 `route-memory-retrieval.js`；canonical 候选目录在 `route-candidates.js`；binding role、clarification slot 和 canonical resource 投影在 `route-resource-binding.js`；`route-service.js` 继续负责编排、上下文、模型解析和执行合同编译。 路由提示词明确话题优先级：`conversation_focus=text` 且输入无图片词汇时，模糊指代默认指最近文字话题，不得仅因历史图片候选存在就判成图片任务。
