...
function testBackgroundSessionsResumeAndShowBusyStateAfterRestore() {
  ...
  assert.ok(app.includes('if(!s?.id&&!n?.id&&!a)return;setSessionBusy(e.id,!0),resumeSessionJobs(e.id)'), 'background sessions with a stored job or pending submit must be marked busy before reconnecting');
  ...
  assert.ok(index.includes('bootstrap-workflow.js?v=2.0.2-gpt5-reasoning') && index.includes('app.js?v=2.1.23-reasoning-persistence'), 'runtime entry assets should receive cache-version updates with the recovery fix');
}
