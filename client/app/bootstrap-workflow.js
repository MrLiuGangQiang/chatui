(function initChatUIAppBootstrapWorkflow(root) {
  ...
  function createBootstrapWorkflow(deps = {}) {
    const {
      closeConfigModal = root?.closeConfigModal,
      document = root?.document,
      persistBeforePageLeave = root?.persistBeforePageLeave,
      window = root?.window || root,
    } = deps;
    const setTimeout = deps.setTimeout || window?.setTimeout?.bind(window) || root?.setTimeout?.bind(root);
    const clearTimeout = deps.clearTimeout || window?.clearTimeout?.bind(window) || root?.clearTimeout?.bind(root);
    ...
    const reconcileRuntimeUpgrade = deps.reconcileRuntimeUpgrade
      || (async () => root?.[Symbol.for('chatui.module-registry.v1')]?.get('runtimeUpgrade')?.reconcileRuntimeUpgrade?.({
        identity: root?.__CHATUI_RUNTIME_IDENTITY || root?.__CHATUI_ENTRY_IDENTITY,
        storage: deps.localStorage || root?.localStorage,
        sessions: deps.state?.sessions || [],
        persistSessionDisplay: deps.persistSessionDisplay || root?.persistSessionDisplay,
        saveSessionMessages: deps.saveSessionMessages || root?.saveSessionMessages,
      }));
    ...
    async function start() {
      ...
      try{
      {
        const {
          ...
          closeReasoningMenu,
          closeSessionDrawer,
          closeSessionImageStylePanel,
          closeSessionModelPanel,
          closeSessionPromptPanel,
          copyConfigField,
          copyImageActionElement,
          downloadImageActionElement,
          enhanceConfigSelects,
          ensureHistoryAnchorNode,
          historyAnchorItemsFromState,
          isSessionBusy,
          loadAppVersion,
          loadConfig,
          loadGlobalImageStyleToSessionInput,
          loadGlobalPromptToSessionInput,
          ...
          openConfigModal,
          openSessionDrawer,
          openSessionImageStylePanel,
          openSessionModelPanel,
          openSessionPromptPanel,
          refreshActiveSessionOnReturn,
          renderActiveSession,
          requestAnimationFrame,
          rerenderVisibleMarkdownMessages,
          resumeActiveOutputFocus,
          resumeBackgroundSessionJobs,
          revealNodeAboveComposer,
          saveConfig,
          saveSessionsMeta,
          saveSessionImageStyle,
          saveSessionModel,
          saveSessionPrompt,
          scheduleAutoResize,
          scrollPromptByWheel,
