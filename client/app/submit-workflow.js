    ...
    async function onSubmit(e) {
      with (deps) {
          ...
          try{
            ...
            let pendingSourceAttachments=[];
            if(storedPending?.sourceAttachmentContext&&typeof restoreUserAttachmentsFromContext==="function")try{
              pendingSourceAttachments=await restoreUserAttachmentsFromContext(storedPending.sourceAttachmentContext)
            }catch(e){console.warn("restore pending source attachments failed",e),pendingSourceAttachments=[]}
            else if(storedPending?.sourceImageContext&&typeof restoreImageAttachmentsFromContext==="function")try{
              pendingSourceAttachments=await restoreImageAttachmentsFromContext(storedPending.sourceImageContext)
            }catch(e){console.warn("restore pending source images failed",e),pendingSourceAttachments=[]}
            const currentTurnAttachments=[...attachments];
            const referencesPreviousFile=/(上次|上一次|之前|历史|刚才).{0,8}(文件|文档|附件)|继续.{0,8}(分析|总结|处理).{0,8}(上次|之前|那个)?(文件|文档|附件)/.test(String(promptText||""));
            if(!attachments.length&&referencesPreviousFile&&typeof restoreUserAttachmentsFromContext==="function"){
              const historyMessages=[...(targetSession.messages||state.messages||[])].reverse();
              const source=historyMessages.find(message=>{
                const context=parseContextValue(message?.attachmentContext);
                return Array.isArray(context?.attachments)&&context.attachments.some(item=>item&&!String(item.type||item.mime||"").startsWith("image/"));
              });
              if(source?.attachmentContext)try{
                const restored=await restoreUserAttachmentsFromContext(source.attachmentContext);
                const files=restored.filter(item=>!isImageAttachment(item));
                if(files.length){attachments=[...files];requestAttachments=quotedImageAttachments.length?[...quotedImageAttachments,...attachments]:attachments}
              }catch(e){console.warn("restore previous file attachments failed",e)}
            }
            const mergedCandidateAttachments=[...pendingSourceAttachments,...currentTurnAttachments];
            let pendingDecision=null,pendingResolvedRoute=null;
            if(storedPending&&clarification.buildContinuationClassifierPayload&&clarification.parseContinuationClassifierResult&&typeof getConfig==="function"&&typeof requestJson==="function"){
              ...
              if(cfg.baseUrl&&model)try{
                ...
                const result=await requestJson(`${cfg.baseUrl}/chat/completions`,payload,cfg.apiKey,{headers:buildRequestHeaders("message",sessionId),signal:run.abortController?.signal});
                pendingDecision=clarification.parseContinuationClassifierResult(result?.choices?.[0]?.message?.content||result?.output_text||"");
              }catch(err){console.warn("continuation classifier failed; treating current input as a new task:",err)}
            }
            const shouldMergePending=["pending_answer","revision","continuation"].includes(pendingDecision?.relation)&&pendingDecision?.shouldMerge===!0;
            const pendingMerge=shouldMergePending?clarification.mergePendingInput(storedPending,{promptText,attachments:mergedCandidateAttachments,quotedMessage,quoteText:quotedCleanText,finalPrompt:pendingDecision?.finalPrompt||"",finalTaskMode:pendingDecision?.finalTaskMode||"",selectedIndexes:pendingDecision?.selectedIndexes||[]}):null;
            ...
            const rerouteAfterClarification=storedPending?.routeInfo?.requiresRerouteAfterClarification===!0;
            if(pendingMerge?.merged){
              promptText=pendingMerge.promptText;
              const structuredTask=storedPending?.routeInfo?.taskContract;
              if(structuredTask&&typeof routeUtils.resolveClarificationRoute==="function"){
                const pendingAttachmentMeta=root?.ChatUICore?.attachments?.buildRouteAttachmentMetadata?.(currentTurnAttachments)||[];
                pendingResolvedRoute=routeUtils.resolveClarificationRoute(structuredTask,pendingDecision?.selections||[],{input:pendingMerge.finalPrompt||promptText,attachments:pendingAttachmentMeta});
                if(pendingResolvedRoute)routeInfo=pendingResolvedRoute,routeMode=pendingResolvedRoute.mode,pendingMerge.finalTaskMode=pendingResolvedRoute.mode;
                else routeInfo={...storedPending.routeInfo,mode:"chat",api:"clarify",intent:"clarify",needClarification:!0},routeMode="chat";
              }
              if(!structuredTask&&!rerouteAfterClarification&&pendingMerge.finalTaskMode==="image")routeInfo=normalizeRoute({mode:"image",target:"new",usePreviousImage:!1,intent:"text_to_image",confidence:pendingDecision?.confidence||1,evidence:pendingDecision?.reason||"任务延续模型返回最小合成 prompt"},"image"),routeMode="image";
              else if(!structuredTask&&!rerouteAfterClarification&&pendingMerge.finalTaskMode==="edit_image")routeInfo=normalizeRoute({mode:"edit_image",target:storedPending.sourceImageContext?"previous":"uploaded",usePreviousImage:!!storedPending.sourceImageContext,selectedIndexes:pendingMerge.selectedIndexes||[],intent:"image_edit",editInstruction:pendingMerge.finalPrompt||promptText,confidence:pendingDecision?.confidence||1,evidence:pendingDecision?.reason||"任务延续模型返回最小合成 prompt"},"edit_image"),routeMode="edit_image";
              if(pendingSourceAttachments.length){attachments=mergedCandidateAttachments;requestAttachments=quotedImageAttachments.length?[...quotedImageAttachments,...attachments]:attachments}
            }
            if(storedPending&&targetSession.pendingClarification){delete targetSession.pendingClarification;sessionForReply&&delete sessionForReply.pendingClarification;saveSessionsMeta?.()}
            ...
            if(hasQuotedMessage&&!pendingMerge?.merged){
              ...
              if(hasQuotedImage&&"edit_image"===routeMode){
                if(!(routeInfo.selectedImageIds?.length||routeInfo.selectedIndexes?.length)){routeInfo=normalizeRoute({mode:"chat",target:"none",usePreviousImage:!1,needClarification:!0,clarificationQuestion:"请明确要修改引用消息中的哪一张或哪几张图片。",intent:"image_edit",editInstruction:routeInfo.editInstruction||promptText,confidence:routeInfo.confidence||.6,evidence:"引用图片编辑未能识别具体图片索引"},"chat"),routeMode="chat"}
                else routeInfo=normalizeRoute({...routeInfo,mode:"edit_image",target:quotedImageSource,selectedReferenceId:quotedReferenceId||routeInfo.selectedReferenceId,usePreviousImage:!1,confidence:routeInfo.confidence||1,evidence:routeInfo.evidence||"引用图片为准，按引用图片执行编辑"},"edit_image"),routeMode="edit_image"
              }else if(!hasQuotedImage&&"edit_image"===routeMode){routeInfo=normalizeRoute({mode:"chat",target:"none",usePreviousImage:!1,confidence:1,evidence:"引用内容不含可编辑图片，改为聊天"},"chat"),routeMode="chat"}
            ...
            else if(!pendingResolvedRoute&&(rerouteAfterClarification||!(pendingMerge?.merged&&["image","edit_image","chat"].includes(pendingMerge.finalTaskMode))))try{routeInfo=await getEffectiveRouteWithSlowNotice(effectivePromptText,requestAttachments,buildRequestHeaders("message",sessionId),null),routeMode=routeInfo.mode}catch(e){throw e}
            if(routeInfo.needClarification){const e=routeInfo.clarificationQuestion||"请问你要编辑哪一张图？可以说第一张、第二张，或选择全部。",t={role:"assistant",content:e,rawText:e,responseIndex};typeof updateMessage==="function"&&assistantNode?.isConnected&&(delete assistantNode.dataset.jobId,updateMessage(assistantNode,e,{rawText:e,responseIndex}));liveItem&&(delete liveItem.jobId,typeof updateSessionDisplayItem==="function"?updateSessionDisplayItem(sessionId,liveItem,"assistant",e,{rawText:e,pending:!1,responseIndex}):(liveItem.content=e,liveItem.rawText=e,liveItem.pending=!1,persistSessionDisplay(sessionId)));if(isTargetActive()){state.messages.push(t);sessionForReply.messages=cloneMessageList(state.messages)}else targetSession.messages=cloneMessageList([...(targetSession.messages||[]),t]);const createdPending=pendingMerge?.merged?{...pendingMerge.pending,clarificationText:e,expects:clarification.expectedAnswerTypes?.({...pendingMerge.pending,clarificationText:e})||pendingMerge.pending.expects,updatedAt:Date.now()}:clarification.createPendingClarification?.({messages:sessionForReply.messages||targetSession.messages||state.messages||[],clarificationText:e,routeInfo,sourceImageContext:imageContext||null,sourceAttachmentContext:attachmentContext||null,sourceQuoteContext:quoteContext||null});if(createdPending&&!routeInfo.localClarification){targetSession.pendingClarification=createdPending;sessionForReply&&(sessionForReply.pendingClarification=createdPending)}await persistPendingTerminalMessages();emitTaskEvent(sessionId,taskEvents.TASK_COMPLETED_COMMITTED,{submissionId});clearPendingSubmit(sessionId);preparedChatJobId&&typeof clearChatJob==="function"&&clearChatJob(sessionId);preparedChatJobId="";saveSessionsMeta?.();return}
            if(submissionCancelled()){clearPendingSubmit(sessionId);return}
            if(routeInfo.taskContract&&routeUtils.isRouteDispatchable?.(routeInfo)!==!0){const e=new Error("路由任务尚未完成资源确认，已停止发送");e.code="ROUTE_NOT_READY";throw e}
            ...
            const routeAttachmentSelectors=submitHelpers.createRouteAttachmentSelectors(routeInfo,{isImageFile:typeof isImageFile==="function"?isImageFile:void 0,isImageUnderstandingChat:()=>submitHelpers.isImageUnderstandingChat(promptText),isFileUnderstandingChat:()=>submitHelpers.isFileUnderstandingChat(promptText),currentTurnAttachments,includeCurrentTurnImages:!0,editFallbackImages:currentTurnAttachments,decorateImage:(item,index)=>({...item,sourceIndex:originalImageIndex(item,index)})});
            const routeImageRefs=routeAttachmentSelectors.imageRefs,selectedQuotedEditAttachments=()=>routeAttachmentSelectors.selectQuotedEditAttachments(quotedImageAttachments,requestAttachments),selectedEditAttachments=routeAttachmentSelectors.selectEditAttachments,selectedImageGenerationAttachments=routeAttachmentSelectors.selectImageGenerationAttachments,selectedChatAttachments=routeAttachmentSelectors.selectChatAttachments;
            const historicalRouteImageRefs=()=>routeImageRefs().filter(ref=>["history","context"].includes(String(ref?.source||""))),historicalRouteImageIds=()=>historicalRouteImageRefs().map(ref=>ref?.image_id||ref?.imageId).filter(Boolean),historicalRouteImageIndexes=()=>historicalRouteImageRefs().map(ref=>Number(ref?.index)).filter(index=>Number.isInteger(index)&&index>=1),historicalRouteReferenceId=()=>{const reference=historicalRouteImageRefs().find(ref=>ref?.reference_id||ref?.referenceId);return reference?.reference_id||reference?.referenceId||""};
            const isMixedHistoryCurrentImageCompare=()=>historicalRouteImageRefs().length>0&&routeImageRefs().some(ref=>String(ref?.source||"")==="current")&&currentTurnAttachments.some(isImageAttachment);
            const needsHistoricalChatImages=()=>"chat"===routeMode&&historicalRouteImageRefs().length>0;
            const buildComparisonSourceAttachments=async()=>{if(!isMixedHistoryCurrentImageCompare()&&!needsHistoricalChatImages())return requestAttachments;if(typeof getPreviousImageAttachments!=="function")throw new Error("无法加载已选择的历史参考图片，不能在未附图的情况下继续执行");const previous=await getPreviousImageAttachments(sessionId,historicalRouteImageIndexes(),historicalRouteReferenceId(),historicalRouteImageIds());if(needsHistoricalChatImages()&&!previous.length)throw new Error("已选择的历史参考图片无法恢复，不能在未附图的情况下继续执行");return previous.length?[...previous,...requestAttachments]:requestAttachments};
            ...
            const imagePrompt=pendingMerge?.finalPrompt||("image"===routeMode&&routeImagePrompt?routeImagePrompt:quotedMessage&&!pendingMerge?.merged&&"image"===routeMode?[quotedCleanText,effectivePromptText].filter(Boolean).join("\n\n"):effectivePromptText),chatAttachmentCandidates=quotedMessage&&!pendingMerge?.merged?routeSelectedQuotedImages():selectedChatAttachments(comparisonSourceAttachments),chatAttachments=await prepareChatImageAttachments(chatAttachmentCandidates);if(needsHistoricalChatImages()&&!chatAttachments.some(isImageAttachment))throw new Error("已选择的历史参考图片无法作为聊天附件发送，不能在未附图的情况下继续执行");const chatPrompt=[comparisonGuide,imageAttachmentIndexGuide(chatAttachments),effectivePromptText].filter(Boolean).join("\n\n"),editAttachments="edit_image"===routeMode&&routeInfo.intent==="image_reference_gen"?selectedImageGenerationAttachments(requestAttachments):quotedMessage&&!pendingMerge?.merged&&"edit_image"===routeMode?selectedQuotedEditAttachments():"edit_image"===routeMode?selectedEditAttachments(requestAttachments):"image"===routeMode?[]:requestAttachments;
            ...
            if(pendingMerge?.merged&&targetSession.pendingClarification){delete targetSession.pendingClarification;sessionForReply&&delete sessionForReply.pendingClarification;saveSessionsMeta?.()}else if(clarification.clearPendingIfResolved?.(routeInfo)&&targetSession.pendingClarification){delete targetSession.pendingClarification;sessionForReply&&delete sessionForReply.pendingClarification;saveSessionsMeta?.()}const executionApi=routeInfo.api||("image"===routeMode?"image_generation":"edit_image"===routeMode?"image_edit":"chat");const dispatchMode=executionApi==="image_generation"?"image":executionApi==="image_edit"?"edit_image":"chat";if(isTargetActive()&&liveItem&&(!assistantNode||!assistantNode.isConnected)&&typeof findMessageNodeByDisplayItem==="function")assistantNode=findMessageNodeByDisplayItem(liveItem)||assistantNode;const replacementResponseIndex=replacement?.responseIndex??(resumePendingSubmit?responseIndex:void 0),completeDurableHandoff=(jobId,jobKind)=>{handoffCommitted=!0;emitTaskEvent(sessionId,taskEvents.HANDOFF_COMMITTED,{submissionId,jobId,jobKind});clearPendingSubmit(sessionId)},completeInterfaceTask=(completion={})=>{const completionSessionId=String(completion.sessionId||""),completionSubmissionId=String(completion.submissionId||""),completionJobId=String(completion.jobId||""),completionJobKind=String(completion.jobKind||"");if(!completionSessionId||!completionSubmissionId||!completionJobId||!completionJobKind)return;if(completionSessionId!==String(sessionId)||completionSubmissionId!==String(submissionId)||completionJobId!==String(activeJobId)||completionJobKind!==String(activeJobKind))return;emitTaskEvent(sessionId,taskEvents.JOB_COMPLETED_COMMITTED,{submissionId,jobId:activeJobId,jobKind:activeJobKind});finishSessionTask(sessionId,{run})};if("chat"===dispatchMode){prepareManagedChatJobForLiveItem("chat");if(!preparedChatJobId)throw new Error("无法创建聊天任务恢复标识，请重试");if(!savePendingSubmit(sessionId,{...loadPendingSubmit(sessionId),jobId:preparedChatJobId,jobKind:"chat",stage:"handoff"}))throw new Error("无法保存任务恢复状态，请清理浏览器存储空间后重试");activeJobId=preparedChatJobId;activeJobKind="chat";emitTaskEvent(sessionId,taskEvents.HANDOFF_PREPARED,{submissionId,jobId:activeJobId,jobKind:activeJobKind});await sendChat(chatPrompt,chatAttachments,assistantNode,{sessionId,userAlreadyAdded:!0,liveItem,replaceAssistantIndex:replacementResponseIndex,requestBaseMessages,quotedMessage:quoteScopedChat?quotedMessage:null,routeContextMessageCount:routeMessageProjection?.protectedMessageCount||0,clientJobId:preparedChatJobId,submissionId,onDurableHandoff:()=>completeDurableHandoff(activeJobId,activeJobKind),onInterfaceCompleted:completeInterfaceTask});completeDurableHandoff(activeJobId,activeJobKind);emitTaskEvent(sessionId,taskEvents.JOB_COMPLETED_COMMITTED,{submissionId,jobId:activeJobId,jobKind:activeJobKind})}else{const preparedImageJobId=typeof makeClientImageJobId==="function"?makeClientImageJobId():`imgjob-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`;if(!savePendingSubmit(sessionId,{...loadPendingSubmit(sessionId),jobId:preparedImageJobId,jobKind:"image",stage:"handoff"}))throw new Error("无法保存任务恢复状态，请清理浏览器存储空间后重试");activeJobId=preparedImageJobId;activeJobKind="image";emitTaskEvent(sessionId,taskEvents.HANDOFF_PREPARED,{submissionId,jobId:activeJobId,jobKind:activeJobKind});preparedChatJobId="";liveItem&&(liveItem.jobId=preparedImageJobId,liveItem.pending="1",persistSessionDisplay(sessionId));assistantNode&&(assistantNode.dataset.jobId=preparedImageJobId,clearPendingFeedback?.(assistantNode),clearReasoning?.(assistantNode));await sendImage(imagePrompt,{loadingNode:assistantNode,editMode:"edit_image"===dispatchMode,editTarget:routeInfo.target,usePreviousImage:routeInfo.usePreviousImage,selectedIndexes:routeInfo.selectedIndexes,selectedReferenceId:routeInfo.selectedReferenceId,selectedImageIds:routeInfo.selectedImageIds,imageIntent:routeInfo.intent,referenceImageBindings:routeImageRefs().filter(ref=>["reference","style_reference"].includes(String(ref?.role||""))).map(ref=>({imageId:ref?.image_id||ref?.imageId||"",source:String(ref?.source||"")})),editInstruction:routeInfo.editInstruction,attachments:editAttachments,imageContext:quotedImageContext||("uploaded"===routeInfo.target?(deps.getUploadedImageContext?deps.getUploadedImageContext(sessionId,routeInfo.selectedReferenceId):getLatestUploadedImageContext(sessionId)):null),routePrompt:imagePrompt,originalPrompt:effectivePromptText,sessionId,userAlreadyAdded:!0,liveItem,replaceAssistantIndex:replacementResponseIndex,submissionId,clientJobId:preparedImageJobId,onDurableHandoff:()=>completeDurableHandoff(activeJobId,activeJobKind),onInterfaceCompleted:completeInterfaceTask});completeDurableHandoff(activeJobId,activeJobKind);emitTaskEvent(sessionId,taskEvents.JOB_COMPLETED_COMMITTED,{submissionId,jobId:activeJobId,jobKind:activeJobKind})}state.editingIndex=null,state.editingNode=null
          }catch(err){
