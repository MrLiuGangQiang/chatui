(function initChatUIRouteService(root) {
  ...
  const ROUTE_SYSTEM_PROMPT = [
    ...
    'relation：new 全新任务；continuation 继续/重试，无实质变化；followup 依赖历史或修改/纠正/补充成果。歧义缺信息时用new或followup，resource_refs=[]，goal保留真实任务+附澄清问题。',
    '规则：修改纠正成果→followup；"再来一次/继续"→continuation；"重做并改成白色"→followup；多候选不确定→relation=new或followup，resource_refs=[]，goal保留真实任务并附澄清问题，不猜。',
    ...
    '资源选择优先级（从高到低，满足P1则不再看P2-P5）：',
    '  P1 current_input中明确指定名称、索引或描述的资源。如"分析第二张图"→选i2；"读取report.pdf"→选对应文件；"对比前两张"→选i1+i2。',
    '  P2 本轮上传(source=current)且与current_input语义匹配的唯一资源：current_input模糊（如"有几个问题""分析一下""看看这个""总结一下"）且仅有1个current文件→file_qa绑该文件；仅有1张current图→image_qa绑该图。多个current资源但current_input模糊视为歧义→resource_refs=[]，goal保留真实任务+附澄清问题。',
    ...
    '通用规则：selected覆盖同角色established；多候选同等合理省略resource_refs，goal写清真实任务+澄清点，不猜；不编造ID；plain_chat/text_to_image不绑图/文件；multimodal_qa须同时绑source+attachment。',
    'goal 是下游执行模型唯一指令，须：完整/直接/可执行、合并基础任务与新增要求、消除"它/这个/刚才/继续"等指代、保留全部约束、不写分析/理由/operation名/资源ID、不依赖未绑定上下文。text_to_image的goal必须是独立完整的画面描述，不依赖任何未绑定上下文即可生成。',
    '正例"将目标图中的猫改为白色，保留构图不变。"歧义时goal保留真实任务+附简短澄清问题，如"将目标图中的猫改为白色。（需澄清：第一张还是第二张？）"',
    '空输入补充：仅一张图→image_qa描述；仅一个文件→file_qa概述；多资源或图文并存→relation=new，resource_refs=[]，goal写清需选择哪个资源。',
  ].join('\n');
