...
浏览器检测到未捕获异常、未处理的 Promise 拒绝、资源加载失败、网络请求失败或非 2xx 响应时，会在 5 秒后打开问题反馈窗口。系统自动把已脱敏的异常信息以及当前会话最近 3 轮用户消息与助手答复填入“复现描述”；提交前可自行检查、修改或删除。关闭反馈窗口后再次打开会恢复尚未提交的表单草稿；主动停止请求以及反馈接口自身失败不会递归触发反馈窗口。
