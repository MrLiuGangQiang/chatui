  ...
  function createRealtimeRenderer(render, options = {}) {
    ...
    return {
      set(next) {
        ...
        clearTimer();
        timer = setTimeout(() => { timer = null; renderNow(pendingValue); }, Math.max(0, minIntervalMs - elapsed));
      },
      flush(next) { return flushValue(next); },
      final(next) { const rendered = flushValue(next); clearTimer(); finalized = true; return rendered; },
      cancel() {
        clearTimer();
        cancelled = true;
        value = '';
