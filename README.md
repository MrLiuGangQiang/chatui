...
- Reasoning is shown in a distinct tinted panel above the final answer whenever the provider returns it.
- The waiting timer stops as soon as the first reasoning or answer chunk arrives.
- Completed reasoning collapses automatically and can be reopened without losing content.
...
- 图片预览。
- 图片压缩：原始大小超过 5 MB 的 JPEG / PNG / WebP 会先压缩，目标不超过 5 MB。
- BMP 转 PNG。
- 图片作为聊天多模态内容。
- 图片作为图片编辑输入。
...
每个附件必须严格小于 10 MB；已有草稿附件与本次新增附件的合计也必须严格小于 10 MB。图片原始大小超过 5 MB 时会先自动压缩，压缩失败或压缩后仍超过 5 MB 时不会加入附件，并在输入区以红色错误提示保留。
...
浏览器检测到未捕获异常、未处理的 Promise 拒绝、API/功能请求网络失败或非 2xx 响应时，不会自动弹出窗口，而是把最近一次已脱敏的异常记录下来，等用户点击顶栏“问题反馈”时自动带入。普通图片、样式和可选依赖等资源加载失败不会被记录，也不会触发反馈。系统自动把已脱敏的异常信息以及当前会话最近 3 轮用户消息与助手答复填入“复现描述”；即使没有发生异常，主动点击问题反馈也会自动填入最近 3 轮会话上下文。提交前可自行检查、修改或删除。关闭反馈窗口后再次打开会恢复尚未提交的表单草稿；主动停止请求以及反馈接口自身失败不会递归触发问题反馈。
