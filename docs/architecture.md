...
- `styles/skin-system.css` 提供右下角切换器共享外壳；`styles/skins/ink/skin.css`、`styles/skins/snow/skin.css`、`styles/skins/serene/skin.css` 通过 `html[data-skin="<id>"]` 作用域承载各自配色、材质与画布（ink 生成式重映射快照与逐皮肤 swatch 色板将在阶段 3 退役/令牌化）。宁静皮肤使用日出湖泊画布、湖绿 `#3f827b` 主色与晨光纸面；ink 与 snow 各自通过本地字体令牌调整排版：ink 使用楷体标题 + Noto Serif SC 500 字重正文的古风栈，snow 使用圆润显示字体 + 清晰 UI 正文；两者只使用本地系统候选，不请求远程字体；皮肤只改视觉，不改布局结构、交互、任务或数据契约。
