...
async function testLongCodeStartsCollapsedAndCopiesFullSource() {
  await withDom(async container => {
    ...
    assert.strictEqual(expand.textContent, '查看完整代码');
    ...
    assert.strictEqual(expand.lastElementChild.textContent, '查看完整代码');
    assert.strictEqual(expand.getAttribute('aria-label'), '查看完整代码');
    ...
    assert.strictEqual(expand.querySelectorAll('.code-expand-toggle-icon path').length, 2);
    assert.strictEqual(expand.getAttribute('aria-label'), '收起代码');
