...
function testDownloadFilenamesUseShanghaiTimestamp() {
  ...
  assert.ok(index.includes('client/ui/file-actions.js?v=1.2.67') && index.includes('client/app/image-actions-workflow.js?v=1.2.68') && index.includes('client/app/image-result-workflow.js?v=1.7.1-image-semantics') && index.includes('app.js?v=2.3.0-auto-incident-feedback') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion'), 'browser cache versions should match the current static bundle');
...
function testChatAnswerStreamingFlushesQuickly() {
  ...
  assert.ok(index.includes('chat-workflow.js?v=1.5.0-native-file-inputs') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion') && bundle.includes("BUNDLE_VERSION = '1.3.160-code-action-motion'"), 'cache-busting versions should be bumped for streaming performance fixes');
}
...
function testEmptySessionRendersPremiumWelcome() {
  ...
  assert.ok(index.includes('./app.js?v=2.3.0-auto-incident-feedback') && index.includes('flat-theme.css?v=2.2.3-code-action-motion'), 'welcome assets should use updated cache versions');
...
function testResumeStreamButtonAnchorsAboveComposer() {
  ...
  assert.ok(index.includes('styles/composer.css?v=1.4.0-native-file-inputs') && index.includes('scroll-focus-workflow.js?v=1.3.33'), 'cache-busting versions should be bumped for resume button positioning fixes');
}
...
function testRegenerateSavesEarlyPendingSubmitBeforeRoute() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('regenerate-workflow.js?v=1.2.2-stable-pending-id') && index.includes('app.js?v=2.3.0-auto-incident-feedback') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion'), 'cache versions should deliver the canonical regenerate workflow');
}
...
function testReasoningCompletesBeforeAnswerStreaming() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('chat-workflow.js?v=1.5.0-native-file-inputs') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion'), 'chat stream reasoning-state fix should bump cache versions');
}
...
function testReasoningUnavailableWhenAnswerStartsWithoutReasoning() {
  ...
  const index = fs.readFileSync(path.join(__dirname, '../../index.html'), 'utf8');
  assert.ok(index.includes('chat-workflow.js?v=1.5.0-native-file-inputs') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion'), 'empty-reasoning stream fix should bump cache versions');
}
...
function testReasoningMenuCloseReleasesFocusBeforeAriaHidden() {
  ...
  assert.ok(index.includes('reasoning-workflow.js?v=1.3.35-reasoning-persistence') && index.includes('composer.css?v=1.4.0-native-file-inputs') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion'), 'reasoning menu changes should bump cache versions');
}
...
function testForceImageButtonOnUserMessages() {
  ...
  assert.ok(index.includes('message-workflow.js?v=1.3.41-stream-settle-binding') && index.includes('regenerate-workflow.js?v=1.2.2-stable-pending-id') && index.includes('app.js?v=2.3.0-auto-incident-feedback') && index.includes('assets/chatui.bundle.css?v=1.3.163-changelog-cards') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion') && index.includes('styles/flat-theme.css?v=2.2.3-code-action-motion'), 'force-image UI and action changes should bump cache-busting versions');
...
function testImagePreviewWheelZoom() {
  ...
  assert.ok(index.includes('image-preview-workflow.js?v=1.2.68-safe-clear') && index.includes('chatui.bundle.js?v=1.3.160-code-action-motion') && index.includes('styles/flat-theme.css?v=2.2.3-code-action-motion'), 'image preview zoom should bump cache-busting versions');
...
function testImageSuccessResultReconciliation() {
  ...
  assert.ok(index.includes('image-result-reconciliation.js?v=1.0.0') && index.indexOf('image-result-reconciliation.js?v=1.0.0') < index.indexOf('app.js?v=2.3.0-auto-incident-feedback'), 'reconciliation module should load before app.js');
  assert.ok(index.includes('image-workflow.js?v=1.6.1-precise-role-prompt') && index.includes('job-resume-workflow.js?v=1.3.0-execution-resources') && index.includes('app.js?v=2.3.0-auto-incident-feedback'), 'image success reconciliation should bump workflow and app cache versions');
}
