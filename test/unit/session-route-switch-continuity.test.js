...
function testSessionSwitchRecoveryRebindsWithoutDuplicateExecution() {
  ...
  assert.ok(index.includes('runs.js?v=1.2.66-session-run-owner') && index.includes('submit-workflow.js?v=1.2.82-canonical-task-state') && index.includes('app.js?v=2.1.29-explicit-stop') && index.includes('chatui.bundle.js?v=1.3.116-explicit-stop'), 'browser cache versions must deliver the session-run ownership fix');
}
