...
The follow-up classifier emits only `pending_continuation.v4`: relationship, a complete natural-language `resolved_input`, validated choice evidence, and optional non-executing assistance. It cannot emit an operation, API, runtime mode, final prompt, media indexes, or dispatch authorization. Resource choice language is kept out of `resolved_input`: ordinals, candidate labels, filenames, and similar wording that only identifies an external candidate remain in `selections`, while every semantic object, property, action, value, and constraint from the original request must be preserved. Thus “把猫的颜色换成红色” remains exactly that after the user selects an external image; only “第二张图” is removed, so it cannot be reinterpreted as an internal grid cell or image region by the execution model. Every merged answer—including a structured resource choice—returns through the complete task router with `clarification_context.v1`. For a pure ambiguous-resource answer, the compiler additionally locks the prior operation, changes, constraints, all already established bindings, every selected binding role, and the exact binding count; a reroute cannot drop the known cat, turn a content reference into a style reference, or add an unselected image. Earlier pending attachments keep their historical or contextual source; only attachments uploaded in the answering turn are `current`.
...
`image_reference_gen` produces a new composition but consumes one or more image inputs, so its transport is `/images/edits` with multipart image files. Its product mode remains image-generation (alongside `text_to_image`); transport selection must not relabel it as `edit_image`. Pure `text_to_image` alone uses `/images/generations`. Multi-image requests carry an internal role map in the exact multipart order. The server validates each position, role, resource key, stable ID, and reference ID against the serialized files, then strips the internal map before contacting the upstream API. Partial media serialization fails before durable handoff rather than silently sending a subset.

Outbound chat budgeting treats the current user request, active system prompt, explicit quote, and route-selected messages as required content. Only ordinary history may be summarized or removed. Required content keeps its original text and chronological order; if it alone exceeds the configured context window, the request fails closed instead of truncating the original requirement or a bound resource.
