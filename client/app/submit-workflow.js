  ...
  function createSubmitWorkflow(deps = {}) {
    ...
    async function runSubmit(e, options = {}) {
      with (deps) {
          ...
          const sessionId=resumePendingSubmit?.sessionId||state.activeSessionId,run=ensureActiveRun(sessionId),submissionId=resumePendingSubmit?.submissionId||jobLifecycle.makeSubmissionId?.()||`submit-${Date.now().toString(36)}-${Math.random().toString(36).slice(2,10)}`,startedAt=resumePendingSubmit?.startedAt||Date.now(),initialAttachmentCount=resumePendingSubmit?Math.max(0,Number(resumePendingSubmit.attachmentCount||0)||0):state.attachments.length,initialEditMessageIndex=resumePendingSubmit?parseOptionalMessageIndex(resumePendingSubmit.editMessageIndex):parseOptionalMessageIndex(state.editingIndex);let activeJobId="",activeJobKind="",handoffCommitted=!1,terminalCommitted=!1,attachments=resumePendingSubmit?[]:[...state.attachments],attachmentRestoreFailure=null,routeUi=null,assistantNode=null,liveItem=null;
