(function initChatUIAppScrollFocusWorkflow(root) {
  ...
  function createScrollFocusWorkflow(deps = {}) {
    ...
    function ensureTailScrollSpace(e=72) {
      with (deps) {
        const t=$("messages"),s=document.querySelector(".composer")?.getBoundingClientRect();if(!t||!s)return;const n=t.getBoundingClientRect(),a=Math.max(0,n.bottom-(s.top-e));t.style.setProperty("--session-tail-scroll-space",`${Math.ceil(a)}px`)
      }
    }
    ...
    function scheduleSessionTailFocusAfterLayout(e={}) {
      with (deps) {
        const t=$("messages");if(!t)return scheduleSessionTailFocus(e);cancelSessionTailFocusAfterLayout();const s=++state.scrollVersion,n=Number.isFinite(e.quietMs)?e.quietMs:160,a=Number.isFinite(e.maxMs)?e.maxMs:2400,i=Number.isFinite(e.stableFrames)?Math.max(1,e.stableFrames):3;let o=null,r=null,l=null,d=null,c=null,m=!1,h=new WeakSet;const u=()=>{const e=document.scrollingElement||document.documentElement;return [t.scrollHeight,t.clientHeight,t.children.length,e?.scrollHeight||0,document.body?.scrollHeight||0,innerHeight||0].join(":")},p=()=>{clearTimeout(o),o=null,clearTimeout(c),c=null,r?.disconnect?.(),l?.disconnect?.(),d&&cancelAnimationFrame(d),d=null,m=!0,sessionTailFocusCleanup=null},g=()=>{if(state.scrollVersion!==s||m)return;p()},f=()=>{if(state.scrollVersion!==s||m)return;setMessagesProgrammaticScroll();focusSessionTail(e);requestAnimationFrame(()=>{state.scrollVersion===s&&!m&&focusSessionTail(e)});setTimeout(()=>{state.scrollVersion===s&&!m&&focusSessionTail(e)},120);setTimeout(g,260)},w=()=>{if(state.scrollVersion!==s||m)return;let e="",t=0;const n=()=>{if(state.scrollVersion!==s||m)return;const s=u();s===e?t+=1:(e=s,t=1);if(t>=i)return f();d=requestAnimationFrame(n)};d=requestAnimationFrame(n)},y=()=>{clearTimeout(o),o=setTimeout(w,n)},S=()=>{if(!r)return;[t,...t.querySelectorAll(".message,.bubble-wrap,.bubble,.content,.reasoning-panel,.markdown-body,img,video,iframe,table,pre,.code-block,.mermaid,.mermaid-block")].forEach(e=>{try{h.has(e)||(h.add(e),r.observe(e))}catch{}})};if("ResizeObserver"in window){r=new ResizeObserver(()=>{state.scrollVersion===s&&y()}),S()}if("MutationObserver"in window){l=new MutationObserver(()=>{if(state.scrollVersion!==s)return;S(),y()}),l.observe(t,{childList:!0,subtree:!0,attributes:!0,attributeFilter:["class","style","src","data-persisted-src","width","height"]})}const v=[...t.querySelectorAll("img")].filter(e=>!e.complete);v.forEach(e=>{try{e.addEventListener("load",y,{once:!0}),e.addEventListener("error",y,{once:!0})}catch{}});document.fonts?.ready?.then?.(()=>{state.scrollVersion===s&&y()}).catch?.(()=>{});requestAnimationFrame(()=>requestAnimationFrame(y));c=setTimeout(()=>{state.scrollVersion===s&&!m&&f()},a);sessionTailFocusCleanup=()=>{clearTimeout(o),clearTimeout(c),r?.disconnect?.(),l?.disconnect?.(),d&&cancelAnimationFrame(d),sessionTailFocusCleanup=null,m=!0};return s
      }
    }
    ...
    function markManualMessageScroll(e) {
      with (deps) {
        const t=$("messages");if(!t)return;if((e?.currentTarget===window||e?.target===window)&&e?.target&&!t.contains(e.target)&&e.target!==document&&e.target!==document.body&&e.target!==document.documentElement)return;const s=e?.type==="wheel"&&Math.abs(Number(e.deltaY||0))>1,n=e?.type==="touchstart"||e?.type==="touchmove",a=e?.type==="pointerdown"||e?.type==="mousedown",i=e?.type==="scroll"&&pointerScrolling,o=e?.type==="scroll"&&Math.abs(t.scrollTop-state.lastMessageScrollTop)>1,r=Date.now()<state.programmaticScrollUntil;if(a){pointerScrolling=!0,clearTimeout(pointerScrollTimer),pointerScrollTimer=setTimeout(()=>{pointerScrolling=!1},1200)}if(s||n||a||!r&&(i||o)){state.streamFocusLocked=!1,state.userScrollLocked=!0,state.autoScrollLocked=!1,state.outputPinSuppressUntil=Date.now()+1500,state.scrollVersion+=1,clearTimeout(scrollTimer)}setTimeout(updateResumeStreamButton,0),setTimeout(updateResumeStreamButton,140),state.lastMessageScrollTop=t.scrollTop
      }
    }

    function getComposerSafeBottom() {
      with (deps) {
        const e=getComputedStyle(document.documentElement).getPropertyValue("--composer-safe-bottom");return window.ChatUI?.scroll?.composerSafeBottom?window.ChatUI.scroll.composerSafeBottom(e,168):(t=parseFloat(e),Number.isFinite(t)?t:168)
      }
    }

    function scrollToBottom(e=!0,t={}) {
      with (deps) {
        const s=$("messages");if(!s)return;if(!e&&!shouldFollowScroll())return;const n=window.matchMedia("(max-width: 640px)").matches,i=Array.isArray(t.settleMs)?t.settleMs:[n?80:160],o=()=>focusSessionTail({threshold:12});state.autoScrollLocked=!0,state.userScrollLocked=!1;const r=++state.scrollVersion;state.programmaticScrollUntil=Date.now()+180;o(),requestAnimationFrame(()=>{state.scrollVersion===r&&o()}),clearTimeout(scrollTimer),i.forEach((e,t)=>{const s=setTimeout(()=>{state.scrollVersion===r&&o()},e);t===i.length-1&&(scrollTimer=s)})
      }
    }
