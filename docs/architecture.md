...
- `shared/image-instruction.js`：作为图片执行指令物化协议 `image_instruction.v1` 的严格 schema。路由输出只决定 operation、资源与任务连续性；在图片 provider 调用前，单图任务必须经该协议物化为可独立执行的 `instruction`，多图父任务也必须先物化为下游规划器唯一可执行的任务说明，或返回 `needs_clarification`。历史对话只能作为物化证据，不能把“按方案A/照你说的/上述内容”等引用语直接透传给图片 provider；物化结果和多图子任务都在执行边界拒绝仍含未解析会话引用的文本，服务端最终执行校验也重复该不变量，绝不猜测或用关键词扩写其内容；
