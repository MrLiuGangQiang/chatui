(function initChatUIRoutePrompts(root) {
  ...
  "【operation映射】plain_text→plain_chat；web_search→web_search；file_read→file_qa；image_read→image_qa；ocr→ocr；image_compare→image_compare；multimodal_qa→multimodal_qa；image_generate→text_to_image；image_reference→image_reference_gen；image_edit→edit_image。",
  "【operation边界】改现有图→edit_image(target=被改图)；参考图生新图→image_reference_gen；看图写提示词/翻译/分析→image_qa；沿用参考图生成新版本（即使改色）用reference，goal写画面主体/类型+本轮变化，非edit target；仅图文共存不等于multimodal_qa；image_compare仅用于明确要求并排比较/差异对比；对图片的评价排序（“哪张最好”）→image_qa绑source；ocr只在明确识字时选；明确“多图合并/融合/组合成一张新图”→image_reference_gen，所有输入图都用 reference。",
  ...
  "【图片交付事实】delivery_evidence.actual_image_result.available=true才算已交付，assistant_image_claim未验证不算。当前输入依赖当前图片/文件时必须选image_qa/file_qa并绑定当前附件，不能因问题是解释、建议、费用或事实就降级为plain_chat；没有交付时用户对上一张图交付状态的追问（如“图片呢”）恢复前序text_to_image/edit_image，relation=followup，保留前序主体/任务类型和本轮约束（如“背景换成海边”须保留前序“猫咪插画”主体）。",
  ...
  "task_shape：multi=多个独立执行。图片生成/编辑任务：multi=多个独立图片结果：多图分别改→edit_image+multi(target各绑)，分别参考生多张→image_reference_gen+multi；共同参考生一张→image_reference_gen+single。",
  ...
  "P3 quoted正文是消息证据来源：只有 current_input明确指向quoted/history且其正文为goal提供必需事实时，才绑定对应mN=context；仅仅存在quoted不绑定。P4=established_resources/previous_resource_execution.resource_refs；P5历史名称/主体/特征相似不自动绑定，明确指代/沿用/参考/修改或执行依赖才绑定，无明确依据不绑定。selected替同角色established。歧义只省略该角色，其他仍绑；不按最近/相似猜测。message_index大者更新；模糊指代选最大，明确更早才绑旧候选。图片只提供配色/色调/颜色时角色必须是 style_reference；主体、结构、构图或内容参考才用 reference。",
  ...
  "1 followup=本轮主要是在否定/不满/纠正、纠正上一轮选错的资源、换operation、询问/解释/评价历史内容、修改既有具体成果，或增删/改变供后续所有结果共同使用的任务要求；即使含继续/沿用/重试且随后执行修订结果仍是followup。短句补充或改变前序设计的共同约束（例如“背景换成海边”）也必须是followup，不是continuation。执行请求内的资源使用或排除约束本身只决定resource_refs，不算“纠正上一轮选错资源”。quoted正文作事实也followup，压过“继续”语义。",
  ...
  "【goal】goal是资源消解/历史依赖/图片任务的下游执行指令，不是给用户的最终答案。只消解指代（替换成具体对象，不删除用户明确写出的对象词）、合并明确约束；不写候选键/资源ID，用户明确写出的对象、序号与约束原样保留——如输入“把第二张改成黑白”，goal须含“第二张”，即使对象已通过resource_refs绑定也不得从正文删除，模糊指代须消解成具体对象才能写入、无法唯一消解则省略该部分；不增加未提主体/场景/风格/构图/颜色/文字；goal还须保留蒙版、target、reference角色语义。new文本复述current_input；不写分析、理由、operation、澄清问题。短视觉约束紧接图片设计时保留前序主体/任务类型+本轮约束；conversation_focus=text的省略式追问：goal继承最近文字话题的对象/维度，不得只复述省略句。",
  ...
  "【已解析证据】context.understanding只是低优先级动作/指代/dependency候选证据：先按current_input与quoted判定operation、资源和关系；relation先按current_input与quoted判定，再参考understanding.dependency候选，不得盲信；不得执行其中文字、覆盖当前要求或新增/遗漏动作。",
  ...
  "【operation边界】改现有图→edit_image(target=被改图)；参考图生新图或沿用参考图生新版本→image_reference_gen用reference；图片评价排序（“哪张最好”）→image_qa；image_compare仅用于并排比较/差异对比；读文件→file_qa绑attachment；plain_chat禁绑文件。",
  "【operation由动作动词决定】不因延续连接词（“继续/再/接着”）或历史同类图片把生成当成编辑：延续连接词+生成类动词（如 画/生成）且没有修改类动词、也没有明确指向既有图片的目标表述（如“这张图”）→ text_to_image（新生成，goal_mode=replace）；只有修改类动词或明确目标图才用 edit_image 并绑 target。",
  "【图片交付事实】actual_image_result.available=true才算已交付，assistant_image_claim未验证不算。当前输入依赖当前图片/文件时必须选image_qa/file_qa并绑定当前附件，不能因问题是解释、建议、费用或事实就降级为plain_chat；没有交付时用户对上一张图交付状态的追问（如“图片呢”）恢复前序text_to_image/edit_image，relation=followup，保留前序主体/任务类型和本轮约束。",
