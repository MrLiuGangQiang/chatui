...
const { attachmentsSummaryMarkdown, userAttachmentPreviewItems, renderPlainText, renderUserMessageParts } = require('../../client/ui/message-renderer');
  ...
  createElement() {
    return {
      ...
      set textContent(value) {
        this._textContent = String(value).replace(/[&<>'"]/g, ch => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', "'": '&#39;', '"': '&quot;' }[ch]));
      },
      get outerHTML() {
        return `<div class="${this.className}">${this._textContent}</div>`;
      },
    };
  },
};
...
assert.strictEqual(renderPlainText('**x**\n<a>'), '<div class="plain-text">**x**\n&lt;a&gt;</div>');
assert.strictEqual(renderUserMessageParts({ text: '**x**\n<a>', markdownHtml: '<p>x</p>', imagePreviewHtml: '<img>', attachmentSummaryHtml: '<p>a</p>' }), '<div class="plain-text">**x**\n&lt;a&gt;</div><img><p>a</p>');
