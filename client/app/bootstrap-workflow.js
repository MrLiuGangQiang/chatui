  ...
  function createBootstrapWorkflow(deps = {}) {
    ...
    async function start() {
      ...
      try{
      {
        ...
        // Prefer an injected feature module so startup remains deterministic in
        // tests/embedded hosts; the registry fallback keeps the browser entry
        // compatible with the existing module-loading order.
        const historyAnchorNav = deps.historyAnchorNav
          || root?.ChatUIApp?.appContext?.getWorkflowModule?.('historyAnchorNav');
        historyAnchorNav?.init?.({messages:$('messages'),nav:$('historyAnchorNav'),document,markManualScroll:markManualMessageScroll,revealNode:revealNodeAboveComposer,getItems:()=>historyAnchorItemsFromState(),ensureItemNode:item=>ensureHistoryAnchorNode(item)});
