...
- `shared/image-instruction.js`：作为图片执行指令物化协议 `image_instruction.v1` 的严格 schema。路由输出只决定 operation、资源与任务连续性；只有 `text_to_image + relation=new + goal_mode=replace`、没有任何资源/消息/dispatch 绑定、且 route goal 已是独立完整指令的任务可直接使用该 canonical goal，避免重复模型调用。其余单图任务与多图父任务都必须先经该协议物化为下游执行器/规划器唯一可执行的任务说明，或返回 `needs_clarification`。物化结果按目标形态分两种：`edit_image`（无论 goal_mode）、`image_reference_gen` 与 `text_to_image+goal_mode=replace` 输出完整自足的 provider 指令；`text_to_image+goal_mode=amend` 只输出相对前序基础要求的增量（基础见 `previous_execution.task_state`，应用物化结果时由任务连续性协议拼回），不得复述基础内容，避免 base 在最终 provider prompt 中重复出现。用户委托创作自由（“随便/你决定/you choose/up to you”）或澄清已回答（`answer_complete=true`）时允许自选具体细节，但输出形态仍按该二分执行：`amend` 仍只写增量，不得借委托输出完整自足指令。历史对话只能作为物化证据，不能把“按方案A/照你说的/上述内容/与之前生成的猫的插画保持一致”等跨轮引用语直接透传给图片 provider（图片 provider 看不到历史轮次；已绑定的输入只能以“参考图”本身命名，不得写“之前/上一张/刚才”）；物化结果、快路径准入判断和多图子任务都在执行边界拒绝仍含未解析会话引用的文本，服务端最终执行校验也重复该不变量，绝不猜测或用关键词扩写其内容；
