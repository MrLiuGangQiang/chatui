    ...
    async function onSubmit(e) {
      with (deps) {
          ...
          try{
            ...
            const routeImageRefs=routeAttachmentSelectors.imageRefs,selectedQuotedEditAttachments=()=>routeAttachmentSelectors.selectQuotedEditAttachments(quotedImageAttachments,requestAttachments),selectedEditAttachments=routeAttachmentSelectors.selectEditAttachments,selectedChatAttachments=routeAttachmentSelectors.selectChatAttachments;
            ...
            const needsHistoricalChatImages=()=>"chat"===routeMode&&routeImageRefs().some(ref=>["history","quoted","context"].includes(String(ref?.source||"")));
            const buildComparisonSourceAttachments=async()=>{if(!isMixedHistoryCurrentImageCompare()&&!needsHistoricalChatImages())return requestAttachments;let previous=[];try{previous=typeof getPreviousImageAttachments==="function"?await getPreviousImageAttachments(sessionId,routeInfo.selectedIndexes,routeInfo.selectedReferenceId,routeInfo.selectedImageIds):[]}catch(err){console.warn("restore selected historical chat images failed",err),previous=[]}return previous.length?[...previous,...requestAttachments]:requestAttachments};
