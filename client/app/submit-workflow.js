(function initChatUIAppSubmitWorkflow(root) {
  ...
  function createSubmitWorkflow(deps = {}) {
    ...
    // A submit must claim its session before any asynchronous preparation. The
    // persisted recovery flow can otherwise race a second form submit and start
    // another route/model pipeline for the same user message.
    const inFlightSubmitSessions = new Set();
    ...
    async function runSubmit(e, options = {}) {
      with (deps) {
          ...
          const submitSessionKey=String(resumePendingSubmit?.sessionId||state.activeSessionId||"");
          if(inFlightSubmitSessions.has(submitSessionKey)){
            const submitter=e?.submitter,isSendButton=submitter?.id==="sendBtn"||submitter?.closest?.("#sendBtn");
            if(isSendButton)return void await stopActiveRun(submitSessionKey);
            return;
          }
          inFlightSubmitSessions.add(submitSessionKey);
          let submitClaimReleased=!1;
          const releaseSubmitClaim=()=>{if(submitClaimReleased)return;submitClaimReleased=!0;inFlightSubmitSessions.delete(submitSessionKey)};
          if(!resumePendingSubmit&&(isSessionBusy(state.activeSessionId)||pendingActiveSubmit)){
            ...
            if(state.suppressNextSubmitStop){releaseSubmitClaim();return void(state.suppressNextSubmitStop=!1)}
            releaseSubmitClaim();return void(s?await stopActiveRun(state.activeSessionId):toast("当前正在处理，点击停止按钮可中断"))
          }
          if(hasPendingUploads()){releaseSubmitClaim();return updateSendAvailability(),void toast("文件还在处理中，请等待完成后再发送")}
          state.suppressNextSubmitStop=!1;
          const rawPromptValue=String((options?.promptOverride!==undefined&&options?.promptOverride!==null&&options?.promptOverride!==""?options.promptOverride:(resumePendingSubmit?.rawPromptText??resumePendingSubmit?.promptText??$("prompt").value))),messageSizeGuard=(root?.ChatUICorePreflightGuards||(typeof window!=='undefined'?window.ChatUICorePreflightGuards:null)||{}).validateMessageSize?.(rawPromptValue);if(messageSizeGuard&&!messageSizeGuard.ok){resumePendingSubmit&&clearPendingSubmit(resumePendingSubmit.sessionId||state.activeSessionId);toast(messageSizeGuard.message||"消息过长，请改为上传文本文件或分段发送");releaseSubmitClaim();return}const rawPromptText=rawPromptValue.trim();
          ...
          if(!promptText&&!state.attachments.length&&!resumeHasInput){releaseSubmitClaim();return}
          ...
          try{
            ...
            const quotedMessage=resumePendingSubmit?.quoteContext?parseContextValue(resumePendingSubmit.quoteContext):(state.editingIndex===null?getQuotedMessage?.():(state.editingQuoteContext?parseContextValue(state.editingQuoteContext):null)),quoteContext=submitHelpers.quoteContextJson(resumePendingSubmit?.quoteContext||quotedMessage||"");
            ...
            if(replacement){const existingUser=(isTargetActive()?state.messages:targetSession.messages||[])[replacement.index];userMessageIdentity=existingUser?.id?{id:existingUser.id,turnId:existingUser.turnId||""}:null}
            if(!replacement&&(!resumePendingSubmit||!resumeUserCommitted)){
              const userHtml=renderUserMessageWithAttachments(promptText,attachments),rawText=buildUserMessageContent(promptText,attachments),apiContent=buildUserApiContent(promptText,attachments),identityApi=root?.ChatUIAppSessionPersistence||{},newIdentity=identityApi.createMessageTurnIdentity?.({sessionId,submissionId,role:"user",sequence:messageIndex})||{id:`message:${sessionId}:${submissionId}:user`,turnId:`turn:${sessionId}:${submissionId}`},message={role:"user",content:apiContent,html:userHtml,rawText,messageIndex,submissionId,...newIdentity};userMessageIdentity=newIdentity;
              ...
              userNode=isTargetActive()?addMessage("user",userHtml,{html:!0,rawText,messageIndex,messageId:newIdentity.id,turnId:newIdentity.turnId,quoteContext,imageContext:initialImageContext,attachmentContext:initialAttachmentContext}):null;
              userDisplayItem=appendSessionDisplayMessage(sessionId,"user",userHtml,{html:!0,rawText,messageIndex,messageId:newIdentity.id,turnId:newIdentity.turnId,quoteContext,imageContext:initialImageContext,attachmentContext:initialAttachmentContext});
              persistSessionDisplay(sessionId);
              if(userNode){userNode.__displayItem=userDisplayItem;userDisplayItem?.id&&(userNode.dataset.displayItemId=userDisplayItem.id);newIdentity.id&&(userNode.dataset.messageId=newIdentity.id);newIdentity.turnId&&(userNode.dataset.turnId=newIdentity.turnId);initialImageContext&&(userNode.dataset.imageContext=initialImageContext);initialAttachmentContext&&(userNode.dataset.attachmentContext=initialAttachmentContext)}
              await persistTargetMessages();resumeUserCommitted=!0
            }
            if(!userMessageIdentity){const committedMessages=isTargetActive()?state.messages:targetSession.messages||[],committedUser=committedMessages.find(message=>message?.role==="user"&&String(message?.submissionId||"")===String(submissionId))||committedMessages.find(message=>message?.role==="user"&&String(message?.messageIndex||"")===String(messageIndex))||null;userMessageIdentity=committedUser?.id?{id:committedUser.id,turnId:committedUser.turnId||""}:null}
            if(submissionCancelled()){clearPendingSubmit(sessionId);return}
            ...
            const finishPreflightReply=async(text,meta={})=>{
              ...
              const identityApi=root?.ChatUIAppSessionPersistence||{},replyIdentity=identityApi.createMessageTurnIdentity?.({sessionId,submissionId,role:"assistant",sequence:responseIndex})||{id:`message:${sessionId}:${submissionId}:assistant`,turnId:userMessageIdentity?.turnId||`turn:${sessionId}:${submissionId}`},msg={role:"assistant",content:rawText,rawText,responseIndex,submissionId,...replyIdentity,...userMessageIdentity?.id?{replyToMessageId:userMessageIdentity.id}:{},...isHtml?{html:displayContent}:{},...clarificationId?{clarificationId}:{}};
            ...
            const quoteAuthorized=routeInfo?.dispatchContract?.context_policy?.quoted===true;
            const quoteProjectionAuthorized=quoteAuthorized&&routeMessageProjection?.usesExplicitQuote===true;
            requestBaseMessages=Array.isArray(resumePendingSubmit?.requestBaseMessages)?resumePendingSubmit.requestBaseMessages:(quoteProjectionAuthorized?routeMessageProjection.messages:(replacement&&isTargetActive()?state.messages.slice(0,replacement.index):null));
            ...
            // A compiled multi-image batch deliberately has no parent dispatch
            // contract or parent prompt: each validated child owns its own prompt.
            // Apply this guard only to the single-image path, otherwise a valid
            // batch fails before it can reach the one batch handoff.
            if("chat"!==routeMode&&!imageBatchPlan&&!imagePrompt){const error=new Error("图片任务缺少已校验的执行指令，已停止发送");error.code="IMAGE_EXECUTION_PROMPT_MISSING";throw error}
