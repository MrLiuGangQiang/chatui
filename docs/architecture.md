...
- `shared/task-continuity.js`：作为图片文字任务连续性的协议事实来源。`task_continuity.v1` 由一个 `base` 和按时间顺序排列的 `amendment` 组成；`replace` 建立新基础任务，`amend` 必须存在并验证前序状态，再追加本轮修订，稳定渲染时明确“后者优先”。只有缺少结构化状态的历史记录可通过显式 legacy adapter 从 `resolved_goal/input` 迁移；一旦记录声明了结构化状态，损坏、额外字段或版本不符都必须失败，不能静默回退旧文本。读取边界可对旧版本意外嵌套的完整渲染包络做无损扁平化，恢复 base 与有序 amendments，但不得丢弃语义字段或回退为普通文本。`image_task_lineage.v1` 为批量结果中的每个 `reference_id` 与图片 ID 保存独立 `task_state`，同一引用的冲突状态直接拒绝；异构批次没有隐式的单一前序任务；
- `shared/image-instruction.js`：作为图片执行指令物化协议 `image_instruction.v1` 的严格 schema 与执行前事实来源。JSON-mode 供应商若省略由 `status` 唯一要求为空的伴生字段（`ready.clarification` 或 `needs_clarification.instruction`），协议解析边界只补空字符串；语义必需字段、额外字段和矛盾值仍失败关闭。路由输出只决定 operation、资源与任务连续性；只有 `text_to_image + relation=new + goal_mode=replace`、没有任何资源/消息/dispatch 绑定、且 `current_input` 本身已是完整可执行画面指令的任务直接以用户原文建立执行任务状态（route goal 仅保留为 `userGoal` 路由元数据），不调用物化模型，也不让浓缩 goal 替代原文。其余单图任务与多图父任务都必须先经该协议物化为下游执行器/规划器唯一可执行的任务说明，或返回 `needs_clarification`。物化结果按目标形态分两种：`edit_image`（无论 goal_mode）、`image_reference_gen` 与 `text_to_image+goal_mode=replace` 输出完整自足的 provider 指令；`text_to_image+goal_mode=amend` 只输出相对前序基础要求的增量（基础见 `previous_execution.task_state`，应用物化结果时由任务连续性协议拼回），不得复述基础内容，避免 base 在最终 provider prompt 中重复出现。用户委托创作自由（“随便/你决定/you choose/up to you”）或澄清已回答（`answer_complete=true`）时允许自选具体细节，但输出形态仍按该二分执行：`amend` 仍只写增量，不得借委托输出完整自足指令。历史对话只能作为物化证据，不能把“按方案A/照你说的/上述内容/与之前生成的猫的插画保持一致”等跨轮引用语直接透传给图片 provider（图片 provider 看不到历史轮次；已绑定的输入只能以“参考图”本身命名，不得写“之前/上一张/刚才”）；物化结果、快路径准入判断和多图子任务都在执行边界拒绝仍含未解析会话引用的文本；同一执行指令内部明确声明的前一步/上一阶段属于已解析的步骤引用，不等同于历史对话引用；服务端最终执行校验也重复该不变量，绝不猜测或用关键词扩写其内容；物化 instruction 必须完整保留 `current_input` 中用户明确写出的对象、数量、顺序、否定/排除与全部细节，改写只允许消解指代和补充自足表述，禁止用省略细节的转写替代原文；
- `client/services/route-service.js`：负责把 canonical route context 投影为模型输入；当前用户消息不得重复进入历史候选，路由窗口内的既有文字消息与经过结构预算裁剪的图片/文件候选全部可见，超限时按明确的容量策略淘汰。引用消息、会话焦点、上一执行资源组和历史候选都只是模型证据，本地不得按关键词、焦点或 lineage 删除候选语义并替模型裁决；仅允许把明显失时效的历史图片候选折叠出直接目录（`conversation_focus=text`、输入无图片词汇、候选距最近对话超过 4 条消息时），明确图片指代仍通过记忆检索恢复。唯一的协议定义例外是 `current_input` 为空且本轮全部图片/文件附件可用：这没有文本语义可猜，浏览器会在请求意图模型前编译为“分析全部当前附件”的确定性路由——仅图片为 `image_qa`、仅文件为 `file_qa`、图文混合为 `multimodal_qa`，绑定每个当前附件并使用固定非空 goal，route 标记 `inputDefault=all_current_attachments`。图片/文件正文和规范资源身份始终保持本地。 跨 operation 的多个必做步骤使用 `multi_task_plan.v1` 拆成独立可执行任务，路由只澄清并让用户一次选择一个执行（一次只做一个），不提供“合并执行”伪选项。任务选择回合把计划声明的媒体候选保留在目录中，选中文件/图片任务时直接携带计划声明的绑定执行，不再重复询问“选哪个文件”。路由输入长度必须复用 `client/core/preflight-guards.js` 的 `MAX_USER_MESSAGE_CHARS`，不得在 service 或 workflow 中复制另一套阈值。
- 图片执行连续性：`route-service.js` 以模型返回的 `goal_mode` 驱动 `task_continuity.v1`，不再用 `relation` 推断文字任务继承。`replace` 的 `goal` 必须完整自足并建立全新状态，即使本轮仍是 `followup`；`amend` 只接受 `text_to_image`/`edit_image` 且必须存在有效前序状态。`text_to_image` 的执行 prompt 使用稳定渲染后的完整任务状态；其中自足直接生成（`new + replace + 无绑定`）以用户 `current_input` 原文建立该状态，路由 goal 仅作 `userGoal` 路由元数据；`edit_image` 只向上游发送当前编辑指令，因为目标图承载视觉基线，同时把完整结构化状态单独持久化；`image_reference_gen` 永远建立 replacement 状态。是否使用旧图仅由模型明确返回的 `resource_refs` 决定，`resource_refs=[]` 不得被本地焦点、最近图片或 legacy invariant 重新补图。状态经生成、结果上下文、会话存储、刷新恢复重新进入 `previous_execution.task_state`；任一持久化边界发现显式损坏状态都失败关闭。
...
- `route-prompts.js`：唯一维护意图路由、理解、语义审查、修复、图片规划和图片指令物化的系统提示词；所有意图节点共享“当前输入（含当前附件）> quoted > understanding/context”的证据权威声明；通过参数化工厂注入 image-plan 任务上限，浏览器入口必须在 route-service.js 之前加载；
...
意图管线的可靠性边界如下：
- 意图识别是非流式、延迟敏感的受限 Responses 分类请求：`route-service.js` 发送 `model`、`input`、显式 `stream: false`、`tool_choice: "none"` 和严格 `text.format`，且不携带 `tools`。它不以 `temperature`、`max_output_tokens` 或 `reasoning: none` 压缩复杂的语义裁决；Schema 只承载验证约束，完整但受控的路由策略保留在系统提示词。浏览器侧只接收 `{ "output_text": ... }`，不接收上游 envelope。路由提示词和请求 envelope 固定声明证据权威顺序：当前输入与当前附件 > `quoted` 引用（只有 current input 明确指向时才补充事实）> `understanding/context`、`conversation_focus`、`previous_execution` 和更早历史；理解结果不得覆盖当前输入或 quoted 已明确的动作、目标和资源。`quoted` 与当前附件在意图层显式区分，二者同时出现仍按当前输入先判定；当前轮存在 current 图片/文件且输入未明确回指历史时，首轮候选目录只发布 current 资源，历史候选留在受限上下文中，避免历史资源与当前附件竞争；明确历史指代或当前轮缺少所需资源类型时才恢复对应历史候选。带附件的组合请求在 goal 中保留全部动作并以 `task_shape=multi` 或澄清承载。兼容端点若明确拒绝 `tool_choice`，工作流只删除该不支持参数并重试一次，绝不增加工具。图片任务规划仍是一次性非流式 JSON Responses 请求。意图识别、图片指令物化和图片任务规划一律只使用 `/responses`；兼容协商只能删除被供应商明确拒绝的参数或降级 structured-output 格式，绝不切换到 `/chat/completions`，也绝不调用 SSE。普通 5xx、网络、限流、超时或其他协议错误直接作为该 Responses 请求的失败处理。普通历史消息在 core 层投影为 240 字符摘录，当前输入保持独立完整字段，显式引用消息由统一 route-context policy 保护，不得被 workflow 二次摘要或截断。这些是请求层容量约束，不改变“路由模型是唯一语义裁决者”的边界。
...
- `client/app/submit-workflow.js` 与 `client/app/regenerate-workflow.js`：共享任务生命周期不变量；停止后不得持久化 assistant 澄清、发起业务 handoff 或提交完成事件，durable handoff 与 terminal event 必须按 submission/job identity 幂等。页面恢复只能重新连接已经由服务端持久化、且 submission/job identity 已验证的 handoff；服务端明确返回 Job 不存在时必须清理本地恢复 owner 并呈现中断态，禁止用本地 payload 重新 POST 新 Job；`accepted`、`captured`、`routing` 或缺少匹配 durable job 的 pending submit 只保留本地恢复线索，启动时必须清除而不能自动重放，以避免刷新后重复调用路由/规划模型。重新生成与编辑重发共用 runSubmit：重新生成只把原用户消息准备成相同的 editing 状态，并以 promptOverride 原文调用 submitWorkflow.onSubmit，不再独立复制路由、资源池、执行和任务生命周期；待处理澄清重放与强制生图保持薄适配器；替换链路的意图识别上下文以被替换用户轮次为边界，只携带其之前的消息与执行证据，被替换旧回复及其后内容不进入路由模型；会话处于 stopping 时点击重新生成必须等待停止终态后继续，其他运行任务占用会话时给出明确提示，不得静默吞掉点击；
...
- `client/app/image-workflow.js` 与 `client/app/media-workflow.js`：图片 Job waiter 的终态只经唯一 `finish` 收口；`client/app/job-workflow.js` 与 `client/services/job-service.js` 的 waiter 同样遵守该收口；页面隐藏不能停止终态轮询，重新可见时必须立即拉取 canonical Job 并更新当前 DOM，禁止服务端已完成但页面仍等待、刷新后才出现。终态收口不得依赖 UI 更新回调成功；回调异常、轮询 404 或连续观察失败必须转为可见错误并释放会话 busy。单图终态必须以 canonical 提交为 DOM 收口的先后边界。远端结果直连读取有 15 秒上限，失败或超时后必须在 60 秒内有界地回退 `/api/image`；只有结果 Blob 写入 IndexedDB 并形成 `image_result.v1` 后才能提交消息和清除 durable job。旧 loading DOM 已脱离时，重绘只能发生在 canonical 提交和最终 display 更新之后；完成收口不得依赖刷新触发。会话切换复用 pending DOM 缓存时必须按最新 messageIndex/responseIndex 重新定位已有节点，缓存只能复用内容和身份，不能保留过期顺序。managed job 创建前收到明确 4xx 时，必须释放 durable snapshot 和 pending 投影并呈现错误态；`server/jobs/image.js` 同时记录不含请求正文的拒绝阶段与 error code。
...
- `client/app/scroll-focus-workflow.js`：浏览器实时输出的唯一滚动提交边界。普通回答、大文本增量渲染和 reasoning 都必须复用同一输出锚点与异步布局回调；回调须校验当前输出所有权并尊重用户手动滚动。`client/app/markdown/browser-streaming-renderer.js` 统一上报代码高亮、展开/折叠和流式表格导致的布局变化，`markdown-live-stream.js` 只是透传，不得复制第二套滚动补偿；
- `client/app/message-workflow.js`：消息操作按钮的唯一生命周期与绑定入口。流式正文和思考全文只保存在节点内存属性中，禁止每个 token 重写完整 `data-raw-text` / `data-reasoning-text` DOM attribute；终态才发布一次，读取方优先使用内存值。 display item 必须缓存已连接的 DOM node，pending 流只在第一帧做 canonical 放置；actions 状态未变化不重绑按钮，reasoning 内容未变化不重渲染。`reconcileMessageActions` 根据消息角色和终态生成唯一的 `data-actions-state`：用户消息发送后立即 `ready`，assistant 未完成统一 `pending`，成功、异常、澄清、停止及任意图片结果完成后统一 `ready`。标准按钮只从 `#messageTemplate` 克隆并由 `bindMessageActions` 绑定；`data-persist`、`data-streaming`、job 等属性只保留执行/持久化语义，不得另行决定显隐。单图、多图、聊天、历史恢复和恢复任务只能调用这一收口；图片工作流只可在同一完成后追加图片专属操作。`ensureMessageActions` 与 `reconcileCompletedMessageUi` 是兼容门面，必须委托同一实现；
...
- `client/app/chat-workflow.js`：最终聊天执行拥有唯一的等待计时器；用户明确选择“不思考”时，`buildResponsesPayload` 必须显式发送 `reasoning: { effort: "none" }`，不能省略字段让供应商回退到默认思考；请求进入 durable handoff 后按固定间隔原位更新“正在处理 已等待 N 秒”，首个非空思考或答案增量到达时立即释放并移除等待提示，不能依赖会话切换、正文开始或页面重绘推进计时；同一响应启动边界还要移除等待阶段专用的请求理解轨迹；
...
- `client/services/stream-checkpoint-store.js`：未终态流任务的有界 `resume_cursor.v1` 存储，负责尾部裁剪、latest-wins 合并和独立 IndexedDB 写入；它只提供 provisional recovery，不得替代服务端 Job 或 canonical message/session store；
...
- `styles/surfaces/<domain>.css` 是皮肤可见视觉策略的归属层。所有基础样式表已全部令牌化（约 1828 个控制点）：皮肤通过设令牌值（粗调）+ 直接规则（细调）控制外观，两者均为正式架构。
- `styles/skin-system.css` 提供右下角切换器共享外壳；`styles/skins/ink/skin.css`、`styles/skins/snow/skin.css`、`styles/skins/serene/skin.css` 通过 `html[data-skin="<id>"]` 作用域承载各自配色、材质与画布（ink 生成式重映射快照与逐皮肤 swatch 色板将在阶段 3 退役/令牌化）。宁静皮肤使用日出湖泊画布、湖绿 `#3f827b` 主色与晨光纸面；ink 与 snow 各自通过本地字体令牌调整排版：ink 使用楷体标题 + Noto Serif SC 500 字重正文的古风栈，snow 使用圆润显示字体 + 清晰 UI 正文；两者只使用本地系统候选，不请求远程字体；左侧会话列表的重命名/删除按钮由基座统一为默认隐藏、悬停会话标题显示（键盘 focus、重命名态与粗指针设备例外），皮肤不得覆盖为常显；皮肤只改视觉，不改布局结构、交互、任务或数据契约。
...
- 门禁：`test/unit/skin-loading.test.js` 冻结「bundle 只含默认皮肤、注入锚点顺序、单条皮肤链接、域控制点有默认值、域规则不泄漏到其他皮肤」；`test/unit/skin-system-integrity.test.js` 保留逐皮肤视觉契约与目录隔离；`test/unit/snow-typography.test.js` 与 `test/unit/ink-typography.test.js` 冻结两套皮肤的排版令牌与无远程字体约束；`test/unit/serene-skin.test.js` 冻结宁静皮肤的画布、色板、表面与字体契约。
...
聊天与图片 Job 的生命周期、内存存储、事件订阅、停止、恢复、reasoning 和流解析位于这里。`server/jobs/common.js` 把 `UPSTREAM_TIMEOUT_MS` 实现为首内容超时：请求创建后到第一段上游 body 内容到达前保留单个看门狗计时器；第一段内容到达后立即永久取消计时并继续刷新活动 Job，后续内容间隔和请求总时长均不再触发超时。完全没有首段内容才按阈值中止。`server/jobs/cancellation.js` 是排队取消、运行中取消和终态保护的唯一事实源：Job 在 `ConcurrencyLimiter` 队列中被停止时必须立即移除 waiter，进入上游前必须再次检查可运行状态，用户停止后的迟到成功或 AbortError 不得把终态改回完成或超时。失败/停止的执行必须释放对应幂等占位；幂等索引按 principal 与 submission/job scope 隔离，并使用完整内容 fingerprint 区分短 key 碰撞。Responses 搜索流由 Job parser 收集 URL citation / sources，在完成事件后统一去重并追加来源 Markdown；Job state 是服务端任务状态的事实来源；公开响应必须通过 compact/public snapshot 输出，不能把内部 buffer、凭据或原始大文件数据暴露给浏览器。
...
浏览器保存会话、草稿、配置和持久化媒体引用；大媒体使用 IndexedDB。`client/app/persistence.js` 对大 Base64 数据 URL 的清理必须采用线性扫描，禁止对整条消息 HTML 使用无界贪婪正则；否则大图预览会在持久化阶段触发 V8 栈溢出。`client/app/runtime-upgrade-workflow.js` 在 runtime source fingerprint 变化时记录新身份，但不先删任务交接状态和 pending UI；恢复流程必须先连接并校验原 Job 与执行合同，只有服务端明确返回不存在或合同不兼容时才清理，已完成历史始终不受影响；`client/services/session-snapshot-recovery.js` 对旧 snapshot shape 做显式迁移后再读取，迁移失败只隔离损坏快照，不阻塞整个会话列表。 会话快照 fallback 只保存有界尾部或最小恢复记录，禁止把完整历史重复序列化到 localStorage。单条消息文本同样有界；配额重试可丢弃旧 compatibility fallback，但不能影响 canonical IndexedDB。`client/services/stream-checkpoint-store.js` 为未终态任务保存有界 `resume_cursor.v1`，每个会话最多一个 latest-wins 待写 cursor；pending checkpoint 不得重复提交完整 session snapshot，服务端 Job 是运行中正文权威来源，tail-only cursor 恢复时必须以 offset 0 重放并由服务端 aggregate 替换 provisional preview。API Key 等敏感配置不得进入备份、Release Notes、日志或模型上下文。服务端 Job 当前以进程内存为主，进程重启后不能假定任务仍存在。默认 principal 也是匿名浏览器身份而非账号登录：同源浏览器自动携带 `HttpOnly` Cookie；独立 API 客户端必须保留 Cookie。本次生产仅支持单实例；sticky session 和共享 Job 存储属于后续独立设计，不得将本次实现部署为无共享 JobStore 的多实例系统；真实用户/组织多租户必须由可验证 JWT/OIDC 等受信任身份适配器提供，不能信任客户端自报 ID。

chat Job 实时恢复按会话隔离：每个会话对其当前 Job 使用一条 `GET /api/chat-jobs/:jobId/events?contentLength=&reasoningLength=` EventSource，同一会话/Job 的多个 waiter 只共享 canonical aggregate，不同会话绝不共用连接。compact chat 帧使用默认 SSE message，只发送 `d/r` 增量、一次 `ft`、终态 `done/e/rt` 或 offset 重置 `z`；任务 ID 只存在于 URL，不得每帧重复 `id/status/error`。终态只关闭本会话连接，旧连接迟到回调必须先校验 epoch/Job 对象，不能接管新任务或造成 pending。
...
统计与反馈在浏览器侧是非模态浮层：全屏定位外壳只负责布局，不绘制页面遮罩，也不接收指针事件；只有各自的卡片参与交互，因此页面未被卡片覆盖的区域保持可用。
...
- `CHATUI_ANNOUNCEMENTS_DIR`（默认 `data/announcements`）是唯一运行期公告目录。`server/services/announcements.service.js` 只读取固定文件 `announcement.md`，并通过 `/api/announcements` 提供；`server/services/announcement-events.service.js` 监听该目录，通过公开只读 `/api/announcements/events` SSE 在连接建立及内容指纹变化时推送同一快照。浏览器启动获取一次快照后保持 EventSource，页面激活时做兜底刷新，不运行 5 分钟轮询。同目录的 `model-recommendation.json` 由 `server/services/model-recommendation.service.js` 读取并通过 `/api/config/public` 下发，客户端用于公告侧栏与欢迎页推荐展示。旧 `docs/announcements` 仅保留源码归档，不进入接口或镜像。
...
- `ROUTE_NODE_SYSTEM_PROMPT` 输出 `route_intent.v3`（含完整示例与 relation/goal_mode/资源角色正反示例，≤7600 字符）；operation 按本轮动作、交付物和必需证据共同决定，模式、历史、连接词和泛化动词都不能单独决定 operation；提示词只引用实际 payload 路径（顶层 `understanding`、`context.delivery_evidence`、`clarification_context.*`），`image_compare` 只接受恰好两张图片；附件问答中，仅图片、仅文件、图片+文件共同回答分别对应 `image_qa`、`file_qa`、`multimodal_qa(source+attachment)`，模型将联合媒体请求降级为单模态时进入修复并不得静默漏绑；
...
- 理解节点返回合法但 `actions=[]` 时视为不可用证据，回退完整版路由节点；`explicitImageResultCount` 只计输出结果单位，数量期望与修复门禁在简单路径与 CoT 路径一致生效；显式输出结果数超过 `image_plan` 结构上限（50）时，在调用规划器前由本地护栏确定性失败关闭（按产品上限提示分批），不再把模型无法满足的数量交给规划器。

剩余工作：goal 自洽、遗漏动作等更高层语义修复；简单路径精简版与复杂路径完整规则集均已上线（COMPACT=FULL），开放进一步收敛复杂路径提示词长度的取舍评估（需真实模型 eval 证明语义无损）。
