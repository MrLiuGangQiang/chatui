...
- `shared/route-intent.js`：作为实时模型边界协议 `route_intent.v2` 的严格 schema 与校验事实来源；`operation` 包含普通聊天、`web_search`、文件/多模态问答和图片操作；模型输出固定为 `operation`、`relation`、`goal`、`resource_refs`、`task_shape` 五个字段，协议版本由 schema 名称承载。实时解析器不接受缺少 `task_shape` 的旧四字段结果；历史 v1 数据只能经过显式 adapter 转成 `task_shape=single`。图片、文件和历史消息统一使用 `iN`、`fN`、`mN` 候选键，该协议不包含 API、最终执行参数、上下文策略、幂等键或规范资源身份。默认 schema 是不可变协议模板；每次请求再基于同一请求实际发布的候选目录实例化约束：`candidate_key` 只能从本轮候选 enum 中选择，空目录在协议模板中令 `resource_refs.maxItems=0`。发送 Responses 严格结构化输出前，`client/services/chat-service.js` 会复制并剥离部分供应商不支持的字符串/数组词法与容量关键字；本地 `route_intent` 校验和资源编译仍完整执行这些约束，未知或不可解析资源一律不得授权 dispatch。应用状态已经能确定的字段域也在请求前收窄，而不是在模型返回后改写；
...
- 图片执行连续性：`client/core/image-route-context.js` 从已完成图片结果保存 application-owned `previous_execution.resolved_goal`（完整文字任务规格，本地最多 4000 字符）；模型侧只收到最多 `ROUTE_INTENT_MAX_GOAL_LENGTH` 的受限副本，且它不是历史用户指令。对于模型判定为 `followup`/`continuation` 的同一 `text_to_image` 或 `edit_image` 任务，`route-service.js` 将该本地规格与本轮 `goal` 增量确定性合成；无旧图的 `text_to_image` 将合成结果写入 dispatch contract，`edit_image` 保持本轮编辑指令并把合成结果持久化为下一轮的 `resolved_goal`。`不参照/不使用旧图` 只禁止图片资源绑定，不能清除文字任务规格；明确放弃原要求、换主题或从零开始时必须返回 `relation=new`，不继承该规格。
...
在请求级 schema 已按确定性应用事实收窄允许值之后，路由模型输出的 `route_intent.v2` 是唯一语义裁决结果：未被收窄的 `operation`、`relation`、`goal`、`resource_refs` 与 `task_shape` 都由模型负责，返回后不得做语义改写。`goal` 只消解指代并合并用户已提出的约束，不得补充未提出的创作细节；有资源绑定、历史依赖或图片任务时执行 resolved goal，普通 `relation=new` 的纯文本任务执行原始输入。`task_shape` 描述本轮是否需要多个独立执行，而非资源数量。`single` 是一次 dispatch 可返回一个可合并结果，多图问答、比较、OCR 和汇总仍为 `single`。`task_shape=multi` 表示多个独立执行；仅图片生成/编辑的 `multi` 是可直接执行的多个独立图片结果，并允许进入 `image_plan.v1`。非图片或跨 API 的 `multi` 仅标记“需要拆分”，不会进入图片规划或授权图片批次；执行门禁保留模型语义并阻止发送，要求用户拆分或重述，不再依赖中文关键词正则。本地编译器只从模型实际看到的候选目录重建绑定，以共享能力注册表校验资源类型、角色、数量和可用性，并生成最终、不可变的 `dispatch_contract.v1`；它不能用正则、关键词、会话焦点或资源顺序覆盖模型结果。上下文策略固定为：有消息绑定时 `bound_only`，`relation=new` 且无绑定时 `none`，其他无精确绑定的情况为 `conversation`。最终发送受模型窗口约束，超限时先丢弃最早的非绑定历史，不生成摘要；精确绑定消息与当前消息必须保留，否则失败关闭。未配置意图模型、模型超时、所有模型不可用或输出不符合协议时一律阻止执行，不得降级为本地路由或普通聊天。

意图管线的可靠性边界如下：
- 意图识别是非流式、延迟敏感的受限 Responses 分类请求：`route-service.js` 发送 `model`、`input`、显式 `stream: false`、`tool_choice: "none"` 和严格 `text.format`，且不携带 `tools`。它不以 `temperature`、`max_output_tokens` 或 `reasoning: none` 压缩复杂的语义裁决；Schema 只承载验证约束，完整但受控的路由策略保留在系统提示词。浏览器侧只接收 `{ "output_text": ... }`，不接收上游 envelope。兼容端点若明确拒绝 `tool_choice`，工作流只删除该不支持参数并重试一次，绝不增加工具。图片任务规划仍是一次性非流式 JSON Responses 请求。`/responses` 是首选 transport；只有当该请求返回精确的 HTTP 500 且错误信息包含 `empty stream chunks` 时，`route-intent-workflow.js` 才把该受限文本 payload 转换为非流式 `/chat/completions` 请求。回退同样强制 `stream: false`、保留 structured-output 兼容序列，并且绝不调用 SSE；普通 5xx、网络、限流、超时或其他协议错误不会触发 transport 切换。普通历史消息在 core 层投影为 240 字符摘录，当前输入保持独立完整字段，显式引用消息由统一 route-context policy 保护，不得被 workflow 二次摘要或截断。这些是请求层容量约束，不改变“路由模型是唯一语义裁决者”的边界。
