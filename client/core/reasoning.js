(function initChatUICoreReasoning(root) {
  ...
  function normalizeReasoningText(value) {
    ...
    if (typeof value === 'object') {
      return normalizeReasoningText(
        value.text || value.content || value.summary || value.reasoning || value.thinking || value.reasoning_content || value.thinking_content || value.reasoning_details || value.output_text || value.delta || value.summary_text || ''
      );
    }
    return String(value || '');
  }
  ...
  function extractStreamDelta(event) {
    ...
    const reasoning = normalizeReasoningText(
      delta.reasoning_content || delta.reasoning || delta.thinking || delta.reasoning_details || delta.thinking_content || delta.delta ||
      message.reasoning_content || message.reasoning || message.thinking || message.reasoning_details || message.thinking_content || message.delta ||
      event?.reasoning_content || event?.reasoning || event?.thinking || event?.reasoning_details || event?.thinking_content || event?.reasoning_delta || event?.thinking_delta || ''
    );
  ...
  const api = Object.freeze({ normalizeReasoningText, normalizeContentText, extractStreamDelta, extractResponsesStreamDelta, reasoningBudgetTokens });
