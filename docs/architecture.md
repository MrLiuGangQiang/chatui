...
- `client/app/chat-workflow.js`：最终聊天执行拥有唯一的等待计时器；请求进入 durable handoff 后按固定间隔原位更新“正在处理 已等待 N 秒”，首个非空思考或答案增量到达时立即释放并移除等待提示，不能依赖会话切换、正文开始或页面重绘推进计时；同一响应启动边界还要移除等待阶段专用的请求理解轨迹；
