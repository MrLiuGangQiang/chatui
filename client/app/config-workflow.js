(function initChatUIAppConfigWorkflow(root) {
  ...
  const defaults = Object.freeze({ baseUrl: DEFAULT_BASE_URL, apiKey: '', managedAccessToken: '', headerParams: [], chatModel: '', routeModel: '', imageModel: '', imageSize: 'auto', systemPrompt: '', imageStylePrompt: '', directMode: false, models: [], context: {}, editingIndex: null, editingNode: null, attachments: [] });
  ...
  function createConfigWorkflow(deps = {}) {
    ...
    const MANAGED_ACCESS_TOKEN_STORAGE_KEY = `${CONFIG_KEY}:managed-access-token`;
    ...
    function readManagedAccessToken(){try{return String(sessionStorage?.getItem(MANAGED_ACCESS_TOKEN_STORAGE_KEY)||"").trim()}catch{return""}}

    function writeManagedAccessToken(e=""){const t=String(e||"").trim();try{t?sessionStorage?.setItem(MANAGED_ACCESS_TOKEN_STORAGE_KEY,t):sessionStorage?.removeItem(MANAGED_ACCESS_TOKEN_STORAGE_KEY)}catch{};window?.ChatUIRequestAuth?.setToken?.(t)}
    ...
    function loadConfig(){const e=readJsonStorage(CONFIG_KEY,readJsonStorage("openapi-chat-image-config",{})),legacyApiKey=String(e.apiKey||""),persistedApiKey=readPersistedApiKey()||readLegacySessionApiKey();legacyApiKey&&delete e.apiKey;const t={...defaults,...e,apiKey:persistedApiKey||legacyApiKey,managedAccessToken:readManagedAccessToken()};t.apiKey&&!readPersistedApiKey()&&writePersistedApiKey(t.apiKey);getElement("baseUrl").value=t.baseUrl||defaults.baseUrl,getElement("baseUrl").readOnly=!1,getElement("apiKey").value=t.apiKey||"",getElement("managedAccessToken")&&(getElement("managedAccessToken").value=t.managedAccessToken||""),window?.ChatUIRequestAuth?.setToken?.(t.managedAccessToken),getElement("imageSize").value=t.imageSize||defaults.imageSize,updateCustomSelect(getElement("imageSize")),getElement("systemPrompt").value=t.systemPrompt||"",getElement("imageStylePrompt")&&(getElement("imageStylePrompt").value=t.imageStylePrompt||""),state.models=Array.isArray(t.models)?t.models:[],state.modelMeta=normalizeModelMeta(state.models,t.modelMeta||{});const n=new Set(state.models),a=n.has(t.chatModel)?t.chatModel:"",i=n.has(t.routeModel)?t.routeModel:"",o=n.has(t.imageModel)?t.imageModel:"";renderModelOptions(a,o,i),(legacyApiKey||t.chatModel!==a||t.routeModel!==i||t.imageModel!==o)&&saveConfig(!0),void loadPublicContext()}

    function getConfig(){const e=readJsonStorage(CONFIG_KEY,{}),baseEl=getElement("baseUrl"),apiEl=getElement("apiKey"),managedAccessTokenEl=getElement("managedAccessToken"),chatEl=getElement("chatModel"),routeEl=getElement("routeModel"),imageEl=getElement("imageModel"),sizeEl=getElement("imageSize"),systemEl=getElement("systemPrompt"),styleEl=getElement("imageStylePrompt");const storedModels=Array.isArray(e.models)?e.models:[],models=Array.isArray(state.models)&&state.models.length?state.models:storedModels,context=state.publicContext&&"object"==typeof state.publicContext?state.publicContext:e.context&&"object"==typeof e.context?e.context:{};return{baseUrl:(baseEl?.value.trim()||DEFAULT_BASE_URL).replace(/\/+$/, ""),apiKey:String(apiEl?.value||readPersistedApiKey()||"").trim(),managedAccessToken:String(managedAccessTokenEl?.value||readManagedAccessToken()||"").trim(),headerParams:normalizeHeaderParamConfig(e.headerParams),chatModel:String(chatEl?.value||e.chatModel||"").trim(),routeModel:String(routeEl?.value||e.routeModel||"").trim(),imageModel:String(imageEl?.value||e.imageModel||"").trim(),imageSize:sizeEl?.value||e.imageSize||defaults.imageSize,systemPrompt:String(systemEl?.value||e.systemPrompt||"").trim(),imageStylePrompt:String(styleEl?.value||e.imageStylePrompt||"").trim(),directMode:!!e.directMode,models,context}}
    ...
    function saveConfig(e=!1){cleanupLegacyConfigCache();const t=getConfig();writePersistedApiKey(t.apiKey),writeManagedAccessToken(t.managedAccessToken),localStorage.setItem(CONFIG_KEY,JSON.stringify({baseUrl:t.baseUrl,headerParams:normalizeHeaderParamConfig(t.headerParams),chatModel:t.chatModel,routeModel:t.routeModel,imageModel:t.imageModel,imageSize:t.imageSize,systemPrompt:t.systemPrompt,imageStylePrompt:t.imageStylePrompt,directMode:!!t.directMode,models:Array.isArray(state.models)?state.models:[],modelMeta:state.modelMeta||{}})),e||closeConfigModal()}
    ...
    return Object.freeze({ readJsonStorage, normalizeModelMeta, setApiKeyVisible, toggleApiKeyVisibility, copyConfigField, readPersistedApiKey, writePersistedApiKey, readManagedAccessToken, writeManagedAccessToken, loadPublicContext, loadConfig, getConfig, normalizeHeaderParamConfig, generateShortUuid, ensureSessionHeaderValues, buildRequestHeaders, cleanupLegacyConfigCache, saveConfig, openConfigModal, closeConfigModal });
  }
