(function initChatUIRoutePrompts(root) {
  ...
  "【图片交付事实】context.delivery_evidence.actual_image_result.available=true才算已交付，context.delivery_evidence.assistant_image_claim未验证不算。当前输入依赖当前图片/文件时必须选image_qa/file_qa并绑定当前附件，不能因问题是解释、建议、费用或事实就降级为plain_chat；没有交付时用户对上一张图交付状态的追问（如“图片呢”）恢复前序text_to_image/edit_image，relation=followup，保留前序主体/任务类型和本轮约束（如“背景换成海边”须保留前序“猫咪插画”主体）。无动词的短名词短语约束（如“蓝色背景”）同样继承前序主体/任务类型，不得只写该短语。",
