    ...
    async function runSubmit(e, options = {}) {
      with (deps) {
          ...
          try{
            ...
            requestBaseMessages=Array.isArray(resumePendingSubmit?.requestBaseMessages)?resumePendingSubmit.requestBaseMessages:(routeMessageProjection?.usesExplicitQuote?routeMessageProjection.messages:(quoteScopedChat?[quotedMessage]:(replacement&&isTargetActive()?state.messages.slice(0,replacement.index):null)));
