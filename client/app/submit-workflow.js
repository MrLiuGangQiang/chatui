    ...
    async function onSubmit(e) {
      with (deps) {
          ...
          try{
            ...
            const selectedEditAttachments=(sourceAttachments=requestAttachments)=>{const source=(sourceAttachments||[]).filter(item=>typeof isImageFile==="function"?isImageFile(item):String(item?.type||item?.file?.type||"").startsWith("image/"));const ids=routeSelectedImageIds(),indexes=routeSelectedIndexes();if(!ids.size&&!indexes.size)return source.length===1?source:[];return source.filter((item,index)=>ids.has(item.imageId||item.image_id)||ids.has(item.id)||indexes.has(index+1))};
            ...
            const imagePrompt="image"===routeMode&&routeImagePrompt?routeImagePrompt:quotedMessage&&"image"===routeMode?[quotedCleanText,promptText].filter(Boolean).join("\n\n"):promptText,chatAttachments=quotedMessage?routeSelectedQuotedImages():selectedChatAttachments(requestAttachments),chatPrompt=[imageAttachmentIndexGuide(chatAttachments),promptText].filter(Boolean).join("\n\n"),editAttachments=quotedMessage&&"edit_image"===routeMode?selectedQuotedEditAttachments():"edit_image"===routeMode?selectedEditAttachments(requestAttachments):quotedMessage&&"image"===routeMode?[]:requestAttachments;
