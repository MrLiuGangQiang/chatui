...
async function testRestoredMasksRejectDistinctDuplicatesAndCollapseIdenticalOnes() {
  const contextApi = makeContextWorkflow();
  const base = {
    mode: 'edit_image',
    attachments: [{ id: 'target-1', name: 'target.png', type: 'image/png', src: 'indexeddb://target-1', routeRole: 'target' }],
  ...
  // Identical duplicate representations of the same mask collapse to one mask.
  const identical = await contextApi.restoreImageAttachmentsFromContext({
    ...base,
    masks: [
  ...
  assert.strictEqual(identical.length, 1, 'identical duplicate mask representations must collapse to one');

  // Two genuinely distinct masks are an invalid persisted state: fail clearly
  // and terminally instead of silently dropping one.
  await assert.rejects(
    contextApi.restoreImageAttachmentsFromContext({
      ...base,
      masks: [
        { id: 'mask-1', name: 'mask.png', type: 'image/png', src: 'indexeddb://mask-1', routeRole: 'mask', routeResourceKey: 'r2' },
    ...
    error => error?.code === 'IMAGE_MASK_CARDINALITY_EXCEEDED' && error?.terminalJob === true,
    'distinct multi-mask state must fail terminally instead of silently dropping a mask',
  );
}


function testTwoMaskRouteClarifiesWithMaskChooserAndAnswerDispatches() {
  const routeService = require('../../client/services/route-service');
  const input = '把这两张蒙版分别应用到天空和地面';
  const attachments = [
    { id: 't1', imageId: 't1', name: 'target.png', type: 'image/png', dataUrl: 'data:image/png;base64,AAAA', referenceId: 'ref-t', sourceIndex: 1 },
    { id: 'm1', imageId: 'm1', name: 'm1.png', type: 'image/png', dataUrl: 'data:image/png;base64,BBBB', referenceId: 'ref-m1', sourceIndex: 2 },
    { id: 'm2', imageId: 'm2', name: 'm2.png', type: 'image/png', dataUrl: 'data:image/png;base64,CCCC', referenceId: 'ref-m2', sourceIndex: 3 },
  ];
  const modelIntent = JSON.stringify({
    operation: 'edit_image', relation: 'new', goal: input, goal_mode: 'replace', task_shape: 'single',
    resource_refs: [
      { candidate_key: 'i1', role: 'target' },
      { candidate_key: 'i2', role: 'mask' },
      { candidate_key: 'i3', role: 'mask' },
    ],
  });

  // A multi-mask edit must never execute: it becomes a mask chooser so the user
  // picks the single mask the provider supports.
  const first = routeService.inspectModelRouteResult(modelIntent, { input, attachments, context: {} });
  assert.strictEqual(first.route.needClarification, true);
  assert.match(first.route.clarificationQuestion, /一张蒙版/);
  const maskSlot = first.route.clarificationSlots.find(slot => slot.role === 'mask');
  assert.ok(maskSlot, 'the clarification must offer a mask role slot');
  assert.strictEqual(maskSlot.choices.length, 2);

  // Picking one mask resolves to a dispatchable single-mask edit.
  const answered = routeService.inspectModelRouteResult(modelIntent, {
    input,
    attachments,
    context: {
      clarification_context: {
        ...
        established_resources: [{ resource_key: 'r1', type: 'image', role: 'target', source: 'current', index: 1, id: 't1', resource_id: 'res:image:t1', reference_id: 'ref-t' }],
        selected_resources: [{ resource_key: 'r2', type: 'image', role: 'mask', source: 'current', index: 2, id: 'm1', resource_id: 'res:image:m1', reference_id: 'ref-m1' }],
      },
    },
  });
  assert.strictEqual(answered.route.needClarification, false);
  assert.strictEqual(answered.route.operationType, 'edit_image');
  assert.deepStrictEqual(answered.route.resources.map(resource => resource.role), ['target', 'mask']);
  assert.strictEqual(routeService.isRouteDispatchable(answered.route), true);
}

module.exports = [
  testTwoMaskRouteClarifiesWithMaskChooserAndAnswerDispatches,
  testRestoredMasksRejectDistinctDuplicatesAndCollapseIdenticalOnes,
