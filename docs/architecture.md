...
`client/core/intent-contract.js` strictly validates the complete v5 shape and derives one canonical execution projection directly. A bounded protocol decoder maps an in-flight v4 response to v5 without changing its operation, resource semantics, or clarification decision; the decoded object must still pass the full v5 shape and candidate-binding checks. Before validation, the route service may only complete the base-message binding for an explicit UI quote (`context.quoted_message`) on a `plain_chat` route; it never invents a missing media resource. Every selected image, file, and message resource must resolve uniquely before execution. After parsing, the browser must not reinterpret a valid contract with local keywords, candidate scoring, or synthesized clarification text.
...
Route responses request strict JSON-Schema output for the complete `task_contract.v5` structure. If a response still fails local validation, the same route model receives one bounded repair request containing the rejected output, validation reason, and required readiness. The orchestration layer independently reads the originally declared readiness and rejects any repair or fallback that changes `needs_clarification` to executable. A valid clarification returns immediately from the primary route and never triggers repair or fallback.
